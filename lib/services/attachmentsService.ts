import { Prisma, type Attachment, type WorkItem } from '@prisma/client';
...
/** The `{ attachments: ... }` revision diff cell for ONE panel add/remove (the 5.5.1 collection shape). */
function attachmentsDiffCell(op: 'added' | 'removed', row: Attachment): Record<string, unknown> {
  return {
    attachments: {
      [op]: [{ attachmentId: row.id, name: row.originalFilename, source: row.source }],
    },
  };
}
