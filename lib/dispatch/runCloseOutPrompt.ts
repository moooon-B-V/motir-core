...
export function assembleRunCloseOutPrompt(src: CloseOutPromptSource): string {
  ...
  const lines = [
    ...
    ...section('HOW', [
      ...
      '     the precondition (the sign-in, role, or data the changed surface needs), local',
      '     setup per repository after checking out its branch (install, migrate, seed,',
      '     run), and ONE click-path for the run. Put EVERY command in its own fenced code',
      '     block — the reviewer copies it with one click. Motir fills in the branch fetch',
      '     itself — do not include it.',
      '   - "repos": one entry per repository the run pushed to — the repository and its',
      '     session branch head as commitSha.',
      '   - "previewPath" when there is one.',
      `   - If any card in this run ${RENDERED_SURFACE_TRIGGER},`,
      '     include the click-path section. Otherwise say in the body why there is none,',
      '     e.g. "no rendered surface changed: a service and its tests".',
