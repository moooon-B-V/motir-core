...
const inputSchema = {
  ...
  commitSha: z
    ...
    .optional()
    .describe(
      'The commit the assets were published from, as 7 to 64 HEX characters — a full object id ' +
        'or an abbreviation of it, never a branch name or "HEAD". Surrounding whitespace and ' +
        ...
        'returns the existing result instead of superseding it — which is why it is stored ' +
        'canonical, so two spellings of one commit are one key and a reviewer mid-review does ' +
        'not lose the version they were answering about. The format is checked; whether the ' +
        'commit EXISTS is not.',
    ),
  producedByKey: z
    .string()
    .optional()
