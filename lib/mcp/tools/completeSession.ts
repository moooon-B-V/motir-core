...
import { sessionBranchField } from './workItemRef';
...
export async function runCompleteSession(
  args: { sessionBranch: string },
  ctx: ServiceContext,
): Promise<CallToolResult> {
  try {
    const result = await workItemsService.completeSession(args.sessionBranch, ctx);
  ...
  server.registerTool(
    COMPLETE_SESSION_TOOL_NAME,
    {
      ...
      description:
        ...
        'branch moves to "Done" and its recorded branch is cleared. Returns a per-item outcome ' +
        '(completed / already done / failed). Honors the workflow rules and the same access ' +
        'checks as the UI.',
      inputSchema,
    },
