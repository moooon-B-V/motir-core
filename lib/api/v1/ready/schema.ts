...
//     the CLI's invocation string as public API. **Reaffirmed by Amendment 9
//     Q2**: a client that needs `targetRepo` reads it from
//     `GET /api/v1/work-items/{key}/dispatch-prompt`, which 11.7 ships.
//
// ⚠️ `assignee.name` USED TO BE ON THIS LIST, and Amendment 9 Q1 removed it.
...
export const readyItemSchema = z.object({
  ...
  /** Who it is assigned to, for a client that renders a name (Amendment 9 Q1). */
