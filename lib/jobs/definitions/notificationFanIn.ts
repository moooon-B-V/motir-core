...
// In-app notification fan-in jobs (Story 5.7 · Subtask 5.7.3). Two thin event
// consumers over ONE fan-in service (notificationFanInService) — the 5.1.6
// mentionNotify / 5.4.5 watcherNotify shape. They are a SECOND consumer of the
// SAME channel-agnostic events the 5.1.6 email job consumes: comment mentions
// ride `work-item/comment.created` (emitted by commentsService, 5.1.2),
// description mentions ride `work-item/mentioned` (emitted by workItemsService,
// 5.1.6). No new emit path; no "also notify in-app" call beside any email send.
//
