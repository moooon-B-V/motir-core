...
/**
 * Story 3.4 — Scrum board (sprint-scoped view).
 *
 ...
 *
 * ⚠️ Cross-epic dependency (the stub: "Depends on Epic 4"). A Scrum board needs
 * SPRINTS, which are Epic 4 (Agile planning): the sprint entity (goal / dates /
 * state planned·active·complete) + the issue→sprint association live in Story
 * **4.1**; story points (for "points remaining") live in Story **4.3**; the
 * sprint lifecycle that makes a sprint *active*, provisions the project's Scrum
 * board, and runs *complete sprint* lives in Story **4.4**. Epic 4 is still a
 * `stubs.ts` stub (no subtasks yet), so 3.4's code subtasks depend at the STORY
 * level on 4.1 / 4.3 / 4.4 — they CANNOT reach the ready set until Epic 4 lands.
 * When Epic 4 is expanded, a re-plan can tighten these to subtask-level deps.
 * This is the honest forward reference, surfaced now so the prerequisite is
 * visible at plan time (NOT discovered at run time — mistake #30).
 *
 ...
 * exists. So subtask 3.4.1 is a `type: design` subtask that EXTENDS
 * `design/boards/` (the sprint header + scrum board states), and EVERY
 * UI-touching code subtask (3.4.3) carries 3.4.1 in `dependsOn` and names the
 ...
 *
 * Expanded from its `stubs.ts` entry per `prodect plan 3.4`. Matches the
 * canonical depth + string-literal style of Stories 3.1 / 3.2 / 3.3.
 */
export const story_3_4: PlanStory = {
  id: '3.4',
  ...
  descriptionMd:
    ...
    'scrum }` enum FOR this story (3.4 adds no enum) and the Kanban surface (3.2) + swimlanes/WIP ' +
    '(3.3) are reused wholesale. 3.4 is the **view layer**: a sprint scope on the projection plus ' +
    ...
    '**Cross-epic dependency — sprints are Epic 4 (decision-ladder rung 1 = mirror Jira; the plan ' +
    'puts agile planning in Epic 4).** A Scrum board is meaningless without sprints, and sprints ' +
    'are Epic 4: the **sprint entity** (goal, start/end, state `planned·active·complete`) + the ' +
    '**issue→sprint association** are Story **4.1** (sprint + backlog data model); **story points** ' +
    '(for "points remaining" + the per-column point totals) are Story **4.3** (estimation); the ' +
    "**sprint lifecycle** — *start* (which sets a sprint `active` and provisions the project's " +
    'Scrum board), the **one-active-sprint-per-board guard rail**, and *complete sprint* (carry-' +
    "over) — is Story **4.4**. Epic 4 is unexpanded (a `stubs.ts` stub with no subtasks), so 3.4's " +
    'code subtasks depend at the STORY level on 4.1 / 4.3 / 4.4. **Consequence: 3.4 stays out of ' +
    'the ready set until Epic 4 lands** — that is correct and intended (the stub already said so). ' +
    'A re-plan after Epic 4 expands can retarget these to specific Epic-4 subtasks.\n\n' +
    '**How a Scrum board comes to exist (Epic-4 provisioning, NOT 3.4).** v1 auto-seeds exactly one ' +
    ...
    'auto-creating it when a team first uses sprints, and setting a sprint `active` — is an Epic-4 ' +
    '(4.1/4.4) responsibility, captured here as the dependency above. **3.4 does NOT add board CRUD** ' +
    '(out of scope, consistent with Story 3.3) and does NOT invent a provisioning path; it RENDERS ' +
    "the sprint-scoped view for a board whose `type` is `scrum`, resolving that board's active " +
    'sprint. If the resolved scrum board has no active sprint, 3.4 shows the no-active-sprint empty ' +
    'state (below) rather than failing.\n\n' +
    ...
    '**burndown chart is explicitly Story 4.5**, NOT here — 3.4 shows the numeric remaining only and ' +
    'leaves a documented seam for 4.5 to add the chart.\n\n' +
    "**The sprint header (the one UI surface, per 3.4.1's design).** Above the reused board: the " +
    ...
    "4.4's**; 3.4 REUSES 4.4's complete-sprint trigger/flow and does NOT rebuild carry-over (if 4.4 " +
    'exposes it as a mountable action, 3.4 mounts it in the header; otherwise 3.4 renders the button ' +
    ...
    'where a sprint is started — 4.2/4.4 — so 3.4 links there rather than starting a sprint itself). ' +
    '**No scrum board** for the project (sprints never enabled): fall through to the existing Kanban ' +
    'board (3.2) — 3.4 only *replaces* the view when a scrum board is resolved. **Loading / error** ' +
    ...
    'the same board component — 3.4 wires none of it again. The ONLY net-new code is the projection ' +
    'sprint-scope + summary (3.4.2) and the sprint-header chrome + scrum page resolution (3.4.3).\n\n' +
    ...
    'and the bounded projection all come from Stories 3.1 + 3.2 + 3.3 and are reused, not rebuilt.',
  verificationRecipeMd:
    '- Pull the Story branch, `pnpm install`, `pnpm prisma migrate dev` (no 3.4 migration — the sprint schema is Epic 4.1, the `BoardType` enum is 3.1.1), `pnpm db:seed`, `pnpm dev`. (Requires Epic 4 — 4.1/4.3/4.4 — merged so a scrum board + an active sprint with points exist to render.)\n' +
  ...
  items: [
    {
      ...
      descriptionMd:
        ...
        'board (Story 3.4)" section) naming the composing primitives, copy, and placement. `--el-*` ' +
        ...
        "asset specifies only the entry point's placement/label, and 3.4 reuses 4.4's flow (or " +
        ...
        '## Acceptance criteria\n\n' +
        '- `design/boards/scrum.mock.html` + a PNG export + a "Scrum board (Story 3.4)" section in `design/boards/design-notes.md` exist; the mockup is built from `components/ui/*` + `--el-*`/element-shape tokens only (no Tier-0 `--color-*`, no raw `rounded-*`/`p-*` for control shape), passes the render checklist (icon viewBox, no nested buttons, prettier), and is AA-safe.\n' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        '**Depends on Epic 4 (story-level forward refs).** Story **4.1** provides the `sprint` entity ' +
        '(goal, startDate, endDate, state `planned·active·complete`) + the issue→sprint association ' +
        '(`workItem.sprintId` or a join — whatever 4.1 ships); Story **4.3** provides the story-point ' +
        'field on issues. Both are unexpanded stubs today, so these are story-level deps; this subtask ' +
        'reads those fields, it does not define them. (When 4.1/4.3 expand, a re-plan can retarget to ' +
        'the exact subtasks.)\n\n' +
        ...
        '**Out of scope here:** any UI (3.4.3); the sprint entity / lifecycle / start-complete (Epic ' +
        '4.1/4.4); the burndown chart aggregate (Story 4.5 — only the numeric remaining is here).\n\n' +
        '## Acceptance criteria\n\n' +
        ...
        '- Story 4.1 (sprint entity + issue→sprint association) + Story 4.3 (story-point field) — the Epic-4 model this reads (story-level dep; fields defined there, read here)\n' +
    ...
    },
    {
      ...
      executor: 'coding_agent',
      estimateMinutes: 30,
      dependsOn: ['3.4.1', '3.4.2', '3.2.6', '4.4'],
      descriptionMd:
        ...
        'board provisioned by Epic 4) it renders the Scrum variant; a `kanban` board renders exactly ' +
        'as today. Which board is resolved when a project has both is an Epic-4 / board-nav concern ' +
        "(out of scope — see story Out-of-scope); 3.4 keys purely off the resolved board's `type`. No " +
        'new navigation wiring (the "Boards" nav + ⌘K entry already point at `/boards`, Story 1.5).\n\n' +
        '**Sprint header.** Above the reused board, render the header from the 3.4.2 `SprintSummaryDto`: ' +
        ...
        '**4.4**: if 4.4 exposes a mountable trigger/action, 3.4 mounts it; otherwise 3.4 renders the ' +
        'button as a seam 4.4 wires (the 3.2 Epic-6 Filter-seam pattern). 3.4 does NOT implement ' +
        ...
        'explainer, and a CTA linking to the **Backlog** (Story 4.2 route) to plan/start one (3.4 does ' +
        ...
        '3.4.2 sprint-scoped projection. Swimlanes/WIP work on the scrum board for free because it is ' +
        ...
        '- The complete-sprint entry point is mounted in the header and REUSES the Story-4.4 flow (or is a 4.4-wired seam); 3.4 does NOT implement carry-over/report.\n' +
        ...
        '- Story 3.4.2 — the `sprint` + `columnPoints` + sprint-scoped projection this binds to; Story 4.4 — the complete-sprint flow the header reuses; Story 4.2 — the Backlog route the empty-state CTA links to\n' +
        '- `components/ui/*` (`Pill`, `Tooltip`, `EmptyState`, `ErrorState`, `Button`) + `design/boards/scrum.mock.html` + design-notes (3.4.1) — the header/state spec\n' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        'cutting Epic-3 journey (drag + WIP + swimlanes + scrum AT SCALE) lives in the Epic test story ' +
        '3.5. The 3.1 API + 3.2/3.3 board are already proven; this proves the sprint-scope + header ' +
        'layer on top.\n\n' +
        '**Component / unit (vitest, real Postgres where DB-touching).** The 3.4.2 projection: scrum ' +
        ...
        '(no load-all) — may overlap 3.4.2, keep the integration assertions here. The 3.4.3 header: ' +
        ...
        'Defers to Story 3.5: the combined drag + WIP + swimlanes + scrum journey at scale and the ' +
        "large-active-sprint board/virtualization E2E (this story's scale proof is the 3.4.2 " +
        'acceptance checks against `db:seed:large`).\n\n' +
        '## Acceptance criteria\n\n' +
        ...
        '- The E2E reuses the real-Postgres harness (`tests/helpers/db.ts` truncation) and the seeded project (active sprint state set in the fixture, since the lifecycle UI is Story 4.4); it does NOT duplicate the at-scale combined journey (Story 3.5).\n\n' +
        '## Context refs\n\n' +
        ...
        '- Story 3.4.2 (projection) + 3.4.3 (header/page) — the units under test; Story 3.5 — the Epic-3 test story this defers the at-scale combined journey to\n' +
