...
/**
 * Story 6.12 (Epic 6) — The Project Square (a SYSTEM-level public-project
 * directory). The cross-org DISCOVERY surface for every project made `public`
 * in 6.11: a top-level gallery / explore page any signed-in Motir account can
 ...
 * (6.11.4). Yue: "a 'project square' where all the public projects are showing
 * on the Motir SYSTEM level."
 *
 * **Where this sits relative to 6.11 (the story it builds ON).** 6.11 made a
 * single project openable — `public` as the 4th `ProjectAccessLevel` (6.11.3),
 * the per-project public read-only VIEW (6.11.4), the public roadmap (6.11.7),
 * and the upvote / activity demand signals (6.11.6). But 6.11 is PER-PROJECT:
 * you reach a public project only if you already hold its share-link. 6.12 is
 ...
 * them without a link. It is the open-source-gallery layer on top of 6.11's
 ...
 * 6.11.3 fixed), and every card click lands on the 6.11.4 public view that
 ...
 *     (14.3). **Plane** (https://plane.so/open-source) + the 6.11 portals
 ...
 *
 * **The rank model (computed from 6.11.6 signals, deterministic + paginatable
 ...
 * 6.11.6 vote rows + project activity, not precomputed into a denormalized
 * column this story has to keep fresh.
 *
 * **The directory EXCLUDES non-public projects (the load-bearing correctness).**
 * The 6.12.2 directory service lists ONLY projects whose access level is
 ...
 * the repository layer, and the 6.11.3 404-not-403 posture for non-public
 * projects is untouched (a non-public project is simply not in the result set,
 * never "forbidden"). The square is a thin DISCOVERY index over the 6.11
 ...
 * by topic (e.g. "developer-tools", "design", "open-source"). 6.12.5 owns the
 ...
 * (6.12.1) is a `design` card producing the multi-panel mock + design-notes
 ...
 * (6.12.6) depends on it and is `blocked`.
 *
 ...
 * 6.11.x (the public access level 6.11.3, the upvote/activity signals 6.11.6)
 * and the SHIPPED 6.1.1 FilterAST search. 6.1.1 is DONE so its dep is satisfied;
 * 6.11.x is being planned (not done) so anything chained behind a 6.11.x id is
 * `blocked`. 6.12.1 (design, `dependsOn: []`) is `planned`; every other card
 * (each chained behind 6.12.2 or 6.11.x) is `blocked`.
 */
export const story_6_12: PlanStory = {
  id: '6.12',
  title: 'The project square (system-level public-project directory)',
  status: 'planned',
  gitBranch: 'feat/PROD-6.12-project-square',
  descriptionMd:
    'The **project square** — a SYSTEM-level directory of every project made ' +
    '**public** in 6.11. A top-level gallery / explore page any signed-in ' +
    ...
    'read-only view (6.11.4). It is the cross-org DISCOVERY surface that 6.11 ' +
    'was missing: 6.11 made each project openable but reachable only by ' +
    'share-link; 6.12 is the place all public projects surface together so an ' +
    'account can FIND them without a link.\n\n' +
    '**The model (locked — see the module header for the full rationale + the ' +
    'verified mirror):**\n\n' +
    '- **A thin DISCOVERY index over 6.11, not a new access system.** The ' +
    'square lists ONLY projects whose access level is already `public` (the ' +
    'single cross-org READ exception 6.11.3 fixed); a private / open / limited ' +
    'project NEVER appears, for any viewer. It adds NO new write, NO new ' +
    'cross-org grant — every card click lands on the 6.11.4 public view that ' +
    ...
    'three public demand signals from 6.11.6 (viewers / upvotes / recent ' +
    ...
    '6.11.6 signals — NOT load-all (a system list could be thousands — finding ' +
    '#57).\n' +
    ...
    '**Scope:** the project-square design (6.12.1); the cross-org public-' +
    'projects directory service, cursor-paginated, card-projection, EXCLUDING ' +
    'non-public projects (6.12.2); search + category/tag filtering (6.12.3); ' +
    'the trending / popular / recent ranking (6.12.4); the public-project ' +
    'categories/tags model + tagging + tag-facet (6.12.5); the project-square ' +
    'UI — gallery + cards + search/filter/sort tabs + the shell entry, each ' +
    'card linking to the 6.11.4 public view (6.12.6); the directory + ranking ' +
    '+ search/filter + pagination tests (6.12.7); the browse → search → ' +
    'sort-by-trending → click-into-public-view e2e (6.12.8).\n\n' +
    '**Out of scope (named so they land in their own story, not here):** ' +
    'ANONYMOUS / logged-out access to the square (inherits 6.11’s ' +
    ...
    '(GitHub Collections — a later editorial layer; 6.12 ships the algorithmic ' +
    ...
    'beyond public projects (the shipped FilterAST stays project-scoped — 6.12 ' +
  ...
  verificationRecipeMd:
    ...
    'project’s 6.11.4 public read-only view (the board / issues / public ' +
    ...
    'over the 6.11 surface, not a second view path.\n' +
    '- `pnpm test` (6.12.7) covers: the directory lists ONLY public projects ' +
  ...
  items: [
    {
      ...
      descriptionMd:
        ...
        'subtask here — 6.12.6 — depends on this card and is `blocked` until ' +
        ...
        'three public stat signals from 6.11.6 (**viewers / upvotes / recent ' +
        'activity**) with their glyphs. Draw the card hover/focus state and ' +
        'the whole-card click affordance (the card links to the 6.11.4 public ' +
        ...
        '`components/ui/*`.\n' +
        '- The card draws name + ORG + description + the three 6.11.6 stats ' +
        '(viewers / upvotes / activity) and a whole-card click to the 6.11.4 ' +
        ...
        'links-to-6.11.4 invariants; AA contrast holds for the stat / tab / ' +
        'tag-chip tints.\n\n' +
        '## Context refs\n\n' +
        '- `scripts/plan-seed/data/story-7.0.ts` § 7.0.1 — the multi-panel ' +
        'design-card shape to mirror.\n' +
        '- `scripts/plan-seed/data/story-6.11.ts` § 6.11.1 — the public-view + ' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        "repository read filtered on `access_level = 'public'` (the 6.11.3 " +
        ...
        'open / limited project NEVER appears, for any viewer; the 6.11.3 ' +
        ...
        'not anonymous, inheriting 6.11’s decision).\n' +
        ...
        'three 6.11.6 public stats (viewer count, upvote total, a recent-' +
        'activity signal) — NEVER an internal project field. The stats are ' +
        'read from the 6.11.6 vote / activity signals (this card surfaces the ' +
        'projection; 6.12.4 owns the ranking ORDER over them).\n' +
        ...
        'skips/duplicates a row — 6.12.4 swaps in the trending/popular/recent ' +
        ...
        'three 6.11.6 stats; no internal project field is present in the ' +
        ...
        '## Context refs\n\n' +
        '- `scripts/plan-seed/data/story-6.11.ts` § 6.11.3 (the `public` ' +
        '`ProjectAccessLevel` value + the cross-org read exception) + § 6.11.6 ' +
        '(the upvote / activity signals the stats read from) + § 6.11.4 (the ' +
        ...
        '6.11.3) — the `public` level the filter keys off.\n' +
    ...
    },
    {
      ...
      estimateMinutes: 50,
      descriptionMd:
        'The search + category/tag narrowing over the 6.12.2 directory — the ' +
        ...
        'search composes with the active rank tab (6.12.4) + the cursor ' +
        'pagination, and is bounded (no load-all of the result).\n' +
        '- **Category / tag filter:** narrow the directory to a category/tag ' +
        '(the 6.12.5 tag model) — an EXISTS-over-the-tag-join predicate that ' +
        'composes with search + the rank under the cursor. (6.12.5 owns the ' +
        ...
        'service method that threads the search/tag predicate into the 6.12.2 ' +
        ...
        'EXISTS over the 6.12.5 tag join); search + tag + the rank tab COMPOSE ' +
        ...
        'compose with 6.12.2’s deterministic order + cursor.\n\n' +
        '## Context refs\n\n' +
        '- 6.12.2 (the cursored directory read this narrows) + 6.12.5 (the ' +
        'tag model the category filter consumes) + 6.12.4 (the rank the search ' +
        'composes with).\n' +
    ...
    },
    {
      ...
      estimateMinutes: 60,
      descriptionMd:
        'The three ranks over the 6.12.2 directory, computed from the 6.11.6 ' +
        'votes + project activity — the GitHub-Trending / GitLab-Explore-tabs ' +
        'set. Each rank is a DETERMINISTIC total order and rides the 6.12.2 ' +
        ...
        'over the 6.11.6 vote rows’ timestamps + the project activity signal, ' +
        'NOT a raw lifetime count.\n' +
        '- **Popular** — LIFETIME demand: total upvotes + total viewers (the ' +
        '"most-starred" tab). A stable order over the lifetime 6.11.6 totals.\n' +
        ...
        'part of the 6.11.3 access foundation’s data — a `madePublicAt` set ' +
        ...
        'id — so the 6.12.2 keyset cursor never skips/duplicates a row across ' +
        'pages. Ranking is computed at the READ layer over the 6.11.6 signals ' +
        ...
        'ONE service method selecting the rank’s sort key over the 6.12.2 ' +
        ...
        'rides the 6.12.2 cursor — paging a rank skips/duplicates no row; no ' +
        'rank loads-all-then-sorts in memory.\n' +
        '- Ranking is computed at the read layer over the 6.11.6 vote/activity ' +
        ...
        'no raw Prisma in the route); composes with the 6.12.3 search/tag ' +
        'filter.\n\n' +
        '## Context refs\n\n' +
        '- 6.12.2 (the cursored directory read the ranks order) + 6.11.6 (the ' +
        'upvote / viewer / activity signals the ranks compute from).\n' +
        '- `scripts/plan-seed/data/story-6.11.ts` § 6.11.6 (the ' +
        '`PublicRequestVote` + activity demand signals) + § 6.11.3 (the access ' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        'tags; the square filters to a tag (6.12.3) and lists the categories ' +
        'sorted by project count.\n\n' +
        ...
        'respects the 6.12.2 `access_level = public` filter, so a tag’s count ' +
        'is its public-project count, and a tag with only non-public projects ' +
        'does not inflate the square). This read feeds the 6.12.3 category ' +
        ...
        'counts (respecting the 6.12.2 public-only filter); it feeds the ' +
        '6.12.3 category filter + the categories-browse view.\n' +
        '- 4-layer respected (the tag model in a single-op repository, the ' +
        'tagging + facet logic in services, no raw Prisma in routes).\n\n' +
        '## Context refs\n\n' +
        '- 6.12.2 (the `access_level = public` filter the tag counts respect) ' +
        '+ 6.12.3 (the category filter this model feeds).\n' +
    ...
    },
    {
      id: '6.12.6',
      title:
        'The project-square UI — gallery + cards + search/filter/sort tabs + the shell entry, each card linking to the 6.11.4 public view',
      status: 'blocked',
      type: 'code',
      executor: 'coding_agent',
      estimateMinutes: 65,
      descriptionMd:
        'Build the project-square surface per the 6.12.1 design, over the ' +
        '6.12.3 search/filter + the 6.12.4 ranking. The system-level gallery a ' +
        'signed-in account browses to discover public projects.\n\n' +
        '- **The gallery + cards:** render the grid of project cards (name, ' +
        'org, description, the three 6.11.6 stats) from the 6.12.2 card ' +
        'projection; each card is a whole-card LINK to that project’s 6.11.4 ' +
        'public read-only view (the discovery → view handoff — no second view ' +
        'path here). Paginated / lazy (the at-scale rule — the "load more" / ' +
        'next-page affordance over the 6.12.2 cursor; never load-all).\n' +
        '- **The sort / rank tabs:** the Trending / Popular / Recent tabs ' +
        '(+ the recency-window selector if designed) driving the 6.12.4 rank; ' +
        ...
        'category/tag filter driving 6.12.3; both carried in URL params that ' +
        'COMPOSE with the rank tab + the cursor; the categories-browse view ' +
        '(topics by count) from the 6.12.5 facet read; the active-filter + ' +
        'clear states.\n' +
        ...
        'through the 6.12.2/6.12.3/6.12.4 services (no view-specific query ' +
        'code, no raw Prisma in the route).\n\n' +
        '## Acceptance criteria\n\n' +
        '- The square renders the card gallery (name / org / description / the ' +
        'three stats) per the 6.12.1 design; each card links to the project’s ' +
        '6.11.4 public view; the gallery is paginated / lazy over the 6.12.2 ' +
        'cursor (not load-all).\n' +
        '- The Trending / Popular / Recent tabs drive the 6.12.4 rank; the ' +
        'search bar + category/tag filter drive 6.12.3; rank + search + tag are ' +
        ...
        '`components/ui/*`; matches the 6.12.1 design; next-intl; 4-layer ' +
        'respected (reads through the services, no raw Prisma in the route).\n\n' +
        '## Context refs\n\n' +
        '- 6.12.1 (the design asset — required), 6.12.3 (search + category/tag ' +
        'filter), 6.12.4 (the rank tabs).\n' +
        '- `scripts/plan-seed/data/story-6.11.ts` § 6.11.4 — the public ' +
        ...
        'threading pattern.',
      dependsOn: ['6.12.1', '6.12.3', '6.12.4'],
    },
    {
      id: '6.12.7',
      title:
      ...
      descriptionMd:
        ...
        'Postgres helper.\n\n' +
        '## Context refs\n\n' +
        '- 6.12.2 (the directory + projection + cursor), 6.12.3 (search + ' +
        'category/tag), 6.12.4 (the ranks), 6.12.5 (the tag-facet count).\n' +
        '- `motir-core/CLAUDE.md` § tests-use-real-Postgres + the per-file ' +
        'coverage gate.\n' +
        '- `motir-core/tests/helpers/db.ts` — the per-test truncation ' +
        'harness.',
      dependsOn: ['6.12.2', '6.12.4'],
    },
    {
      id: '6.12.8',
      title:
      ...
      descriptionMd:
        ...
        'state).\n' +
        '4. **Click a card** → it lands on that project’s 6.11.4 public ' +
        ...
        '/ internal comments) are absent — the 6.11 projection still holds ' +
        ...
        '- Clicking a card lands on the project’s 6.11.4 public view with no ' +
        ...
        '## Context refs\n\n' +
        '- 6.12.6 (the square UI driven) — the gallery + tabs + search/filter ' +
        '+ the card link; 6.12.2/6.12.3/6.12.4 exercised through it.\n' +
        '- `scripts/plan-seed/data/story-6.11.ts` § 6.11.10 — the public-view ' +
        'e2e whose harness + selectors this builds on (the card click lands in ' +
        'the 6.11.4 view it drives).\n' +
