...
/**
 * Story 6.9 — The Organization (root-account) tier + org admin. Motir today
 ...
 * project+workspace+org for rollups. **6.9 does NOT do that re-keying and does
 * NOT depend on 7.12** — 6.9 ships the `Organization` MODEL + the org ADMIN
 ...
 *
 * **Scope boundary (what 6.9 is / is NOT).** 6.9 IS: the `Organization` +
 ...
 * loader modelling the `moooon` org; vitest + e2e. 6.9 is NOT: the customer
 ...
 * checkout (Epic 8).
 *
 * **The design gate fires (Principle #13).** 6.9 ships real user-facing
 * surfaces — the org switcher in the shell, org settings, and cross-workspace
 * member management. So the FIRST subtask (6.9.1) is a `design` card producing
 * `design/org-admin/*.mock.html` + `design-notes.md`, and the UI-touching code
 * subtask (6.9.5) depends on it and is `blocked` behind it.
 *
 * **Cross-story dep audit (notes.html #32): PASSES — NO forward deps.** Every
 * 6.9 leaf depends only on same-story 6.9.x cards (story number 6.9 ≤ 6.9). It
 * touches NOTHING in 7.x or 10.x. The billing-entity-is-the-org fact is recorded
 * in PROSE (the orchestrator re-keys 7.12, not this story); 6.9 carries no
 * `dependsOn` on 7.12/7.x/anything > 6.9. Statuses follow the rule: the design
 * card (6.9.1) and the decision card (6.9.2) have `dependsOn: []` → `planned`;
...
export const story_6_9: PlanStory = {
  ...
  status: 'planned',
  gitBranch: 'feat/PROD-6.9-organization-tier-admin',
  descriptionMd:
    ...
    '6.9 establishes that identity + the admin, while the org-scoped credit/' +
    'usage VIEW is a later story (7.12.5) and the cross-org platform console is ' +
    'Epic 10 — both deliberately out of 6.9.\n\n' +
    ...
    'top tier with no parent; 6.9 adds `Workspace.organizationId` so every ' +
    ...
    '(the orchestrator re-keys 7.12’s ledger to the org separately — 6.9 ' +
    'records the decision, ships no credit view, and does NOT depend on ' +
    '7.12).\n\n' +
    '**Scope:** the org-admin design (6.9.1); the `Organization`-model + ' +
    'billing-entity + role decision (6.9.2); the schema + migration + backfill ' +
    'every-workspace→a-default-org (6.9.3); the org-scoped services + access ' +
    'gating (6.9.4); the org admin UI — settings + cross-workspace members + ' +
    'the shell org switcher (6.9.5); the seed loader modelling the `moooon` ' +
    'org (6.9.6); vitest (6.9.7); e2e (6.9.8).\n\n' +
  ...
  verificationRecipeMd:
    ...
    '- `pnpm test` (6.9.7) covers the org model + the membership-gating ' +
    ...
    '- The e2e (6.9.8) creates an org, attaches workspaces, and manages a ' +
    'cross-workspace member end to end.\n' +
    '- **Dep audit.** Confirm no 6.9 subtask references any id > 6.9 (no ' +
  ...
  items: [
    {
      ...
      descriptionMd:
        ...
        'design-reference rule). The org admin UI (6.9.5) depends on this ' +
        ...
        '**Branch.** `design/PROD-6.9.1-org-admin`. The `design/*` prefix gate ' +
    ...
    },
    {
      ...
      estimateMinutes: 45,
      descriptionMd:
        '**Type:** decision (the keystone ADR the schema [6.9.3], the gating ' +
        '[6.9.4], and every later org-aware story build against). Produce a ' +
        ...
        'that **6.9 ships no credit view and takes no dep on 7.12** (the ' +
        ...
        'not asserted).\n' +
        '- It states plainly that the org is the billing entity AND that 6.9 ' +
        ...
        '- 6.9.1 — the design surfaces this decision’s roles/gating drive.\n' +
        '- Story 7.12 (stub) — the credit ledger the orchestrator re-keys to ' +
        'the org (NOT a dep of 6.9); Epic 10 (stub) — the cross-org platform ' +
    ...
    },
    {
      ...
      estimateMinutes: 65,
      descriptionMd:
        'Implement the schema for the org tier decided in 6.9.2, with a ' +
        ...
        '(6.9.4), the UI (6.9.5), and the seed (6.9.6) build on.\n\n' +
        ...
        'org-scoped role from 6.9.2, distinct from the 6.4 `MemberRole`).\n' +
        ...
        'CLAUDE.md. NO business logic here (that is 6.9.4’s service).\n\n' +
        '## Acceptance criteria\n\n' +
        ...
        '## Context refs\n\n' +
        '- 6.9.2 — the model + backfill decision this implements.\n' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        'the business logic the org admin API/UI (6.9.5) and the platform ' +
        ...
        'gates workspace access — 6.9.2). Compose the roles per 6.9.2’s ' +
        ...
        'errors; routes (added in 6.9.5) call exactly one service method.\n' +
        ...
        '## Context refs\n\n' +
        '- 6.9.2 — the gating + role-precedence decision this implements.\n' +
        '- 6.9.3 — the `organizationRepository` / ' +
    ...
    },
    {
      ...
      estimateMinutes: 70,
      descriptionMd:
        'Build the org administration UI from the 6.9.1 design, over the ' +
        '6.9.4 services: the org switcher in the app shell, the org settings ' +
        ...
        'calls ONE `organizationsService` method (6.9.4); the route is ' +
        'session-gated (401 without a session) + org-gated (404-not-403 for a ' +
        'non-org-member, via the 6.9.4 gate). No `db.*` / `$transaction` / ' +
        'business logic in routes.\n\n' +
        '**The surfaces (render the 6.9.1 design verbatim):**\n\n' +
        ...
        'role-changes members — all rendering the 6.9.1 design.\n' +
        ...
        'subtask).\n\n' +
        '## Context refs\n\n' +
        '- 6.9.1 — the design asset (the surfaces this implements verbatim).\n' +
        '- 6.9.4 — the `organizationsService` + the access gate this UI calls ' +
        'through.\n' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        'hierarchy. This keeps the seeded world consistent with the 6.9.3 ' +
        'schema + the 6.9.4 gating (every seeded workspace lives under an ' +
        ...
        'management UI (6.9.5) + the e2e (6.9.8) have realistic data.\n' +
        '- Apply the SAME backfill rule the 6.9.3 migration uses (idempotent: ' +
        ...
        'memberships) and agrees with the 6.9.3 migration backfill.\n' +
        '- The seeded world satisfies the 6.9.4 gate (no seeded workspace is ' +
        'orphaned from an org; the seed owner can administer it as org ' +
        'owner).\n\n' +
        '## Context refs\n\n' +
        '- 6.9.3 — the schema + the backfill rule the seed mirrors ' +
        '(idempotent).\n' +
        '- `motir-core/scripts/plan-seed/seed.ts` — the loader to extend (it ' +
        'already builds the `moooon`/`motir` workspace + project tree).\n' +
        '- 6.9.4 — the gate the seeded data must satisfy.\n' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        'fails).\n\n' +
        '## Context refs\n\n' +
        '- 6.9.3 / 6.9.4 (the schema + gating under test).\n' +
    ...
    },
    {
      ...
      descriptionMd:
        ...
        '## Context refs\n\n' +
        '- 6.9.5 — the org admin UI under test.\n' +
        '- `motir-core/e2e/` — the existing Playwright specs + the ' +
        'run-harness + selector conventions to mirror.\n' +
        '- 6.9.6 — the seeded `moooon` org + members the flow runs against.',
      dependsOn: ['6.9.5'],
    },
  ],
};
