...
// THE READS' `tx ?? dbRead` FALLBACK (MOTIR-5687).
//
// ⚠️ THIS GAP IS OLDER THAN THE CARD THAT EXPOSED IT, and that is the only
// reason it is worth a comment. `approvalGateRepository.ts` carried its own
// copy of a SQLSTATE extractor until MOTIR-5687 moved it to `lib/prisma/sqlstate.ts`
// (one home, after a Prisma upgrade moved the SQLSTATE and silently retired the
// arm in all three copies at once). That helper's branches were FULLY covered,
// so removing them took numerator and denominator away together and dropped the
// file's branch ratio from comfortably over its 85% floor to 84.37% — without
// one line of this file's behaviour changing. The padding left; the gap it was
// hiding is what these arms are.
//
// What they pin is a real claim rather than a number: **the workspace GUC rides
// on the TRANSACTION**, so a read handed no `tx` resolves through `dbRead` with
// no `app.workspace_id` set, and the RLS policy hides every row. These methods
// are only meaningful inside `withWorkspaceContext`, and that is asserted here
// rather than assumed — a future caller reaching for the no-`tx` form gets a
// silent empty answer, not an error, which is exactly the shape that hides.
describe('the read methods default to `dbRead` — and see NOTHING outside a workspace transaction', () => {
  it('finds by id, by work item and by kind inside the context, and nothing outside it', async () => {
    const item = await workItemsService.createWorkItem(
      { projectId: fx.projectId, kind: 'task', title: 'Gate reads' },
      fx.ctx,
    );
    ...
    // Inside the context — the `tx` arm, and the control that proves the rows exist.
    await withWorkspaceContext(fx.ctx, async (tx) => {
      expect(await approvalGateRepository.findById(gate.id, tx)).not.toBeNull();
      expect(await approvalGateRepository.findAwaitingByWorkItem(item.id, tx)).toHaveLength(1);
      expect(
        await approvalGateRepository.findLatestByWorkItem(item.id, 'design_result', tx),
      ).not.toBeNull();
    });

    // Outside it — the `dbRead` arm. Same rows, no GUC, nothing visible.
    expect(await approvalGateRepository.findById(gate.id)).toBeNull();
    expect(await approvalGateRepository.findAwaitingByWorkItem(item.id)).toEqual([]);
    expect(await approvalGateRepository.findLatestByWorkItem(item.id, 'design_result')).toBeNull();
    expect(
      await approvalGateRepository.findLatestApprovedByWorkItems([item.id], 'design_result'),
    ).toEqual(new Map());
  });

  it('answers an EMPTY id list without asking the database at all', async () => {
    // The early return above the client resolve — a batch read is called with
    // whatever the caller's page held, and an empty page must not become a
    // `WHERE id IN ()`.
    expect(
      await approvalGateRepository.findLatestApprovedByWorkItems([], 'pull_request_approval'),
    ).toEqual(new Map());
  });
});
