...
// The project's PLANNER-BUG destination pointer, to a FOLDER — Story MOTIR-5818 ·
// Subtask MOTIR-5820. It mirrors the product pointer
// (`bugDestinationFolderColumn.test.ts`) field for field, and so does this suite.
//
// The column has no readers yet, so everything provable about it is provable
// only against the DATABASE:
//
//   1. `NULL` IS A VALUE — nullable, no default, read from the CATALOG. It means
//      *fall back to the product destination*, never *broken*.
//   2. THE DELETE RULE IS `NO ACTION` — the service carries the pointer up
//      (MOTIR-5821), so a raw delete of the folder it names FAILS.
//   3. THE SAME-PROJECT TRIGGER ADMITS, THEN REFUSES.
//   4. RLS COVERS THE COLUMN WITHOUT A NEW POLICY — admit first, then refuse.
//   5. THE TWO POINTERS ARE INDEPENDENT — both may name one folder, or differ.
...
async function seedTenant(tag: string): Promise<Tenant> {
  ...
  const user = await adminDb.user.create({
    data: { name: `User ${tag}`, email: `planner-bug-dest-folder-${tag}-${n}@example.com` },
  });
  const org = await adminDb.organization.create({
    data: { name: `Org ${tag}`, slug: `pbdf-org-${tag}-${n}` },
  });
  await adminDb.organizationMembership.create({
    data: { organizationId: org.id, userId: user.id, role: 'owner' },
  });
  const workspace = await adminDb.workspace.create({
    data: { name: `WS ${tag}`, slug: `pbdf-ws-${tag}-${n}`, organizationId: org.id },
  });
  ...
  const project = await adminDb.project.create({
    data: {
      name: `Project ${tag}`,
      slug: `pbdf-p-${tag}-${n}`,
      identifier: `PBDF${tag.toUpperCase()}${n}`,
...
async function destinationOf(projectId: string) {
  ...
  return row.plannerBugDestinationFolderId;
}
...
describe('the column, read from the catalog', () => {
  it('is nullable and carries NO default — `null` is the FALLBACK, not an unset field', async () => {
    const rows = await adminDb.$queryRaw<
      Array<{ is_nullable: string; column_default: string | null; data_type: string }>
    >`
      SELECT is_nullable, column_default, data_type
        FROM information_schema.columns
       WHERE table_name = 'project' AND column_name = 'planner_bug_destination_folder_id'
    `;
  ...
  it('carries the index a folder delete scans `project` with', async () => {
    const rows = await adminDb.$queryRaw<Array<{ indexname: string }>>`
      SELECT indexname FROM pg_indexes
       WHERE tablename = 'project' AND indexdef LIKE '%planner_bug_destination_folder_id%'
    `;
    expect(rows.map((r) => r.indexname)).toContain('project_planner_bug_destination_folder_id_idx');
  });
});
...
describe('the same-project trigger', () => {
  it('accepts a ROOT folder and a NESTED folder of its OWN project', async () => {
    ...
    // And back to the fallback, which is always legal.
  ...
  it('rejects a folder of ANOTHER PROJECT in the same workspace', async () => {
    ...
    const sibling = await adminDb.project.create({
      data: {
        name: 'Sibling',
        slug: `pbdf-sibling-${seq++}`,
        identifier: `PBDFS${seq}`,
        workspaceId: home.workspaceId,
      },
    });

    await expect(pointAt(sibling.id, home.bugsFolderId)).rejects.toThrow(
      /PROJECT_PLANNER_BUG_DESTINATION_FOLDER_CROSS_PROJECT/,
    );
  ...
  it('rejects a folder of another WORKSPACE, and names the coarser boundary', async () => {
    await expect(pointAt(home.projectId, neighbour.bugsFolderId)).rejects.toThrow(
      /PROJECT_PLANNER_BUG_DESTINATION_FOLDER_CROSS_WORKSPACE/,
    );
    expect(await destinationOf(home.projectId)).toBeNull();
  });
  ...
  it('refuses a folder id that does not exist, via the foreign key', async () => {
    ...
    await expect(pointAt(home.projectId, 'cmzzzzzzzzzzzzzzzzzzzzzzzz')).rejects.toThrow(
      /project_planner_bug_destination_folder_id_fkey|Foreign key constraint/i,
    );
  });
});
  ...
  it('lets the bound workspace write it', async () => {
    ...
    await withWorkspaceContext({ userId: home.userId, workspaceId: home.workspaceId }, (tx) =>
      tx.project.update({
        ...
        data: { plannerBugDestinationFolderId: home.bugsFolderId },
      }),
    );
    ...
    await expect(
      withWorkspaceContext({ userId: home.userId, workspaceId: home.workspaceId }, (tx) =>
        tx.project.update({
          ...
          data: { plannerBugDestinationFolderId: neighbour.bugsFolderId },
        }),
      ),
    ).rejects.toThrow();
...
describe('the two pointers are independent', () => {
  it('lets both name the SAME folder, or DIFFERENT ones, without either moving the other', async () => {
    await adminDb.project.update({
      where: { id: home.projectId },
      data: {
        bugDestinationFolderId: home.bugsFolderId,
        plannerBugDestinationFolderId: home.nestedFolderId,
      },
    });
    let row = await adminDb.project.findUniqueOrThrow({ where: { id: home.projectId } });
    expect(row.bugDestinationFolderId).toBe(home.bugsFolderId);
    expect(row.plannerBugDestinationFolderId).toBe(home.nestedFolderId);

    await pointAt(home.projectId, home.bugsFolderId);
    row = await adminDb.project.findUniqueOrThrow({ where: { id: home.projectId } });
    expect(row.bugDestinationFolderId).toBe(home.bugsFolderId);
    expect(row.plannerBugDestinationFolderId).toBe(home.bugsFolderId);

    await pointAt(home.projectId, null);
    row = await adminDb.project.findUniqueOrThrow({ where: { id: home.projectId } });
    expect(row.bugDestinationFolderId).toBe(home.bugsFolderId);
    expect(row.plannerBugDestinationFolderId).toBeNull();
  });
});
