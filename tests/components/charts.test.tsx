...
describe('BarChart — 4.6.7 branch pass', () => {
  ...
  it('formats value labels via valueFormat — "—" for an event-less bucket (8.8.13)', () => {
    const { container } = render(
      <BarChart
        series={[{ label: 'Average age', color: chartColor.age }]}
        // The middle bucket is event-less (0) — valueFormat renders it as "—".
        groups={[
          { label: 'Jan', values: [14] },
          { label: 'Feb', values: [0] },
          { label: 'Mar', values: [21] },
        ]}
        yTicks={[0, 10, 20].map((v) => ({ value: v, label: String(v) }))}
        description="Average age with an em-dash for the empty bucket."
        valueFormat={(value) => (value === 0 ? '—' : String(value))}
      />,
    );
    // The "—" appears as a bar value label (the 4.5.2 rule, never a misleading 0).
    const svg = container.querySelector('svg') as SVGElement;
    expect(within(svg as unknown as HTMLElement).getAllByText('—').length).toBeGreaterThan(0);
    expect(within(svg as unknown as HTMLElement).getByText('14')).toBeTruthy();
  });
});
