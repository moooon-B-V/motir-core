...
describe('the Workbench tab strip', () => {
  ...
  it('RENDERS the design order, each tab keeping its own glyph (MOTIR-5217)', async () => {
    const { container } = renderRaw(await WorkbenchTabs({ active: 'todo', counts }));
    // Asserted on the rendered anchors in DOCUMENT order, not on the constant: a
    // strip that re-ordered `WORKBENCH_TABS` but not its own specs would pass a
    // constant check and still render the old order.
