...
// 7.8.3): GET/POST `/api/me/api-tokens` + DELETE `/api/me/api-tokens/[tokenId]`.
// Real Postgres (no mocks — the repo testing contract); only `getSession` is
// stubbed (the cookie the test env can't supply — the sanctioned exception).
// The routes are SESSION-only (the mint surface must never be PAT-reachable),
// scoped to the session user, with the 404-not-403 cross-user contract enforced
// in the service. RLS is inert under the BYPASSRLS test role, so the `db`
...
describe('GET /api/me/api-tokens', () => {
  ...
  it("lists ONLY the session user's tokens, newest first, never the hash", async () => {
    ...
    await apiTokensService.create(alice.id, { label: 'a1' });
    await apiTokensService.create(alice.id, { label: 'a2' });
    await apiTokensService.create(bob.id, { label: 'b1' });
...
describe('POST /api/me/api-tokens', () => {
  it('401 when signed out', async () => {
    expect((await POST(postReq({ label: 'x' }))).status).toBe(401);
  });

  it('creates a token, returns the plaintext ONCE + the row, persists only a hash', async () => {
    const alice = await makeUser('alice@example.com');
    signInAs(alice);

    const res = await POST(postReq({ label: 'claude-code', expiresInDays: 90 }));
    expect(res.status).toBe(201);
    const body = (await res.json()) as {
      token: string;
      dto: { id: string; label: string; expiresAt: string | null };
    };
  ...
  it('null expiresInDays mints a never-expiring token', async () => {
    const alice = await makeUser('alice@example.com');
    signInAs(alice);
    const res = await POST(postReq({ label: 'forever', expiresInDays: null }));
    expect(res.status).toBe(201);
  ...
  it('400 on a non-allowed expiry horizon', async () => {
    const alice = await makeUser('alice@example.com');
    signInAs(alice);
    expect((await POST(postReq({ label: 'x', expiresInDays: 7 }))).status).toBe(400);
  });

  it('400 on a missing label', async () => {
    const alice = await makeUser('alice@example.com');
    signInAs(alice);
    expect((await POST(postReq({ expiresInDays: 90 }))).status).toBe(400);
  });
});
...
describe('DELETE /api/me/api-tokens/[tokenId]', () => {
  ...
  it("soft-revokes the user's own token and returns the revoked DTO", async () => {
    const alice = await makeUser('alice@example.com');
    signInAs(alice);
    const { dto } = await apiTokensService.create(alice.id, { label: 'to-revoke' });
  ...
  it("404 (not 403) revoking another user's token — no existence leak", async () => {
    const alice = await makeUser('alice@example.com');
    const bob = await makeUser('bob@example.com');
    const { dto } = await apiTokensService.create(bob.id, { label: 'bobs' });
