...
describe('the `system.*` schedule is CLUSTERED — the quiet gap the compute sleeps in', () => {
  ...
  it('is the shape the PR body and §21 quote — two clustered minutes, a 30-minute gap', () => {
    ...
    // quickly a stalled container could in principle be caught. It was 18 since
    ...
    // 25 since `system.monitor-issue-reconcile` (MOTIR-4929 · MOTIR-5581), at
    // `0,30 * * * *` — the fifth job on both clustered minutes, costing NO NEW WAKE.
    // Derived by the grep above.
    expect(jobSchedules().length).toBe(25);
