...
const KNOWN: { file: string; address: string; why: string }[] = [
  ...
  // difference is worth reading once. The rows above are prose that never was
  // an address, or an address drawn AHEAD of its route. These two are addresses
  // that were live and correct on the day the asset was drawn, and moved
  // afterwards — and the asset is deliberately NOT being corrected.
  //
  ...
  //
  // ⚠️ These rows carry NO "delete me in card X" instruction, unlike the
  // forward-looking `docs-index.mock.html` row above. There is nothing to come
  // back for: the asset is finished history, and the row is permanent.
  {
