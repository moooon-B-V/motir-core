...
// Pure unit tests (no DB) for the Story 6.8 HTTP-mapping contract: every typed
// error maps to its documented status, and an unknown error returns null (the
// route rethrows → 500).
//
// The preset-avatar registry this file also covered is GONE (MOTIR-2680), and
// its `describe` block went with it — the whole subject was deleted, not the
// coverage of a surviving one. `InvalidProjectImageError` is what the mark can
// fail on now, and it is exercised below.
