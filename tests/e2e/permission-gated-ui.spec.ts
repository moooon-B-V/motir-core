...
// MOTIR-5278 had inverted this to "offered ONE settings room — Approvals, read-only".
// The Approvals refusal below is new with the revert: it is the room this member could
// once read.
test('a MEMBER is offered no settings area — and the room is still shut', async ({ page }) => {
  ...
  await expect(palette.getByRole('option', { name: /Approvals/ })).toHaveCount(0);
  ...
  // ⚠️ RESTORED 2026-09-13 — the Approvals room is manage-only (MOTIR-4880 re-plan ·
  // MOTIR-5394); MOTIR-5278's browse view is reverted.
  // Approvals is the room MOTIR-5278 let this member READ. It is shut like every
  // other now: the typed URL meets the guard, and the stored switch state is not
  // rendered behind the refusal.
  await page.goto('/settings/project/approvals');
  await expect(page.getByRole('heading', { name: 'Admins only' })).toBeVisible();
  await expect(
    page.getByRole('paragraph').filter({ hasText: /which approvals this project asks for/i }),
  ).toBeVisible();
  await expect(page.getByRole('switch', { name: 'Acceptance video approval' })).toHaveCount(0);
  ...
  // ⚠️ RESTORED 2026-09-13 — the Approvals room is manage-only (MOTIR-4880 re-plan ·
  // MOTIR-5394); MOTIR-5278's browse view is reverted.
  // MOTIR-5278 had sent this back to Approvals.
