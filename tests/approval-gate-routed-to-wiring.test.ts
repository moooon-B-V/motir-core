...
describe('every production mount of the approval frame NAMES who it is waiting on', () => {
  it('finds the mount sites at all — the guard must not pass vacuously', () => {
    ...
    // the assertion that turns that into a failure instead. FOUR sites today:
    // the item page's design-result section, the approval OVERLAY — which
    // replaced the Approvals tab's in-list frame when the row began opening it
    // (Story MOTIR-5214 · MOTIR-5225) — the Development block's
    // approve-and-merge frame (MOTIR-5336), and the item page's ACCEPTANCE
    // section, which drew a DECIDED acceptance with a success pill of its own
    // until MOTIR-5792 put it on the shared frame like every other kind.
    expect(MOUNT_SITES.map((s) => s.file).sort()).toEqual([
      'app/(authed)/items/[key]/_components/AcceptancePanel.tsx',
