...
### Panel 6 — an org Admin inside a workspace (design gate, 2026-09-26)

The first publish came back with three questions: can an org Admin leave a workspace, how do they
get back, and how does an Admin who made themselves a Member become a Manager again. The second round
answered them under reading R1 (Admins hold only their workspace role), and the owner then **decided
the model instead**: _an org Admin should always be a Manager in the workspace — carry the org admin
role to the workspace, as other applications do._ Checked against GitHub (org owners are admins of
every repository), Atlassian (org admins are the top role, with automatic product access) and Slack
Enterprise (org owners and admins manage every workspace); recorded as `role-model.md` **AMENDMENT 1**
(2026-09-26), and built in this story (`resolveWorkspaceAccess`, `composeOwnerReach`, the switcher, the
never-wider check). The drawing follows it:

- **6a — the org Admin's own row, and Leave.** Their role picker is **locked at Manager** with the
  reason and where it changes (_Organization Admin — a Manager in every workspace of {org}. Change it
  on the organization's Members page._), for them and for every other Manager: no workspace Manager
  can narrow them. Their **Leave** is disabled with the same reason. So an org Admin never leaves,
  never needs a way back, and cannot demote themselves by accident — the three questions answered at
  the root rather than by a recovery flow.
- **6b — the org Workspaces list.** Every row gains an **Open** link; a workspace the Admin is not on
  the roster of carries a **Manager · via organization** pill (`--el-role-admin`). Opening it writes
  nothing to its roster. The switcher lists every workspace of the org to an Admin, as to the Owner.
- **6c — a Manager who is NOT an org Admin steps down.** The confirmation stays, and now names who can
  restore them: another Manager, an org Admin, or the Owner (an org Admin is a Manager of every
  workspace, so they assign its roles).
...
| `danger.orgAdminLeave`                                     | You’re an organization Admin, so you’re a Manager of every workspace in {org}. To step out of {workspace}, your organization role has to change.                                                                                                | 你是组织管理员，因此是 {org} 所有工作区的管理者。若要退出 {workspace}，需要更改你的组织角色。                                                                                         |
| `members.orgAdminRole`                                     | Organization Admin — a Manager in every workspace of {org}. Change it on the organization’s Members page.                                                                                                                                       | 组织管理员——{org} 所有工作区的管理者。请在组织的成员页面更改。                                                                                                                        |
...
| `workspaces.open` / `workspaces.viaOrganization`           | Open / Manager · via organization                                                                                                                                                                                                               | 打开 / 管理者 · 来自组织                                                                                                                                                              |
| `members.selfDemoteTitle` / `…Body` / `…Keep` / `…Confirm` | Make yourself a {role}? / You won’t be able to change roles in {workspace} any more — including your own. To become a Manager again, ask another Manager of {workspace}, an organization Admin, or the Owner. / Keep Manager / Make me a {role} | 将自己设为{role}？ / 你将无法再更改 {workspace} 中的角色——包括你自己的角色。若要重新成为管理者，请联系 {workspace} 的另一位管理者、组织管理员或所有者。 / 保持管理者 / 将我设为{role} |
