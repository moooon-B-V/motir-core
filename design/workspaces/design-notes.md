...
### Panel 6 — the reviewer's questions (design gate, 2026-09-26)

The first publish was sent back with three questions about an org **Admin** inside a workspace. All
three are answered by the DECISION as approved (`role-model.md` §1, **reading R1 confirmed
2026-09-25**; the _Every role_ table): an org Admin creates and removes workspaces and holds the org
settings, and **inside a workspace holds only the workspace role they hold there**; a workspace role
is granted **by a Manager of that workspace, or by the org Owner**. The drawing makes each answer
visible instead of leaving it to be discovered:

- **6a — Can an org Admin leave a workspace?** Yes, like anyone, through the shipped **Leave**. It now
  confirms first, and the confirmation says what leaving costs them (no reach inside the workspace
  once out) and names the way back. **The last Manager cannot leave** — the button takes the in-place
  disabled treatment, with the reason and what unlocks it (the same rule as 1c/1d).
- **6b — How does the Admin get back?** The shipped org **Workspaces** card lists every workspace to
  an Admin; one they are not in gains a neutral **Not a member** pill and a line naming its Managers.
  They are invited back through the shipped **Invite**, by a Manager or the Owner. **There is no
  “Join” button, deliberately**: a self-join would hand an org Admin exactly the workspace reach R1
  removed.
- **6c — An Admin who makes themselves a Member — how do they become a Manager again?** They cannot
  do it themselves, so the product says so BEFORE it happens: a narrower role picked on your OWN row
  opens a confirmation naming who can restore it (another Manager of the workspace, or the org Owner).
  The last Manager's picker is locked (1c), so the dialog can never leave a workspace with no Manager.

⚠️ **If the answer you want is different — an org Admin who can rejoin, or restore themselves, on
their own — that is an amendment to reading R1, not a drawing choice.** Request changes and say so;
the drawing is two elements (a Join action in 6b, a self-restore in 6c), and the server rule behind
them moves from "Manager or Owner" to "Manager, Owner or org Admin".
...
| `danger.leaveConfirmTitle` / `…Body`                       | Leave {workspace}? / Your organization role does not reach inside a workspace: once you leave, you see none of {workspace}’s projects. To come back, a Manager of {workspace} invites you again — {managers} — or the organization’s Owner adds you. | 离开 {workspace}？ / 你的组织角色不会延伸到工作区内部：离开后，你将看不到 {workspace} 的任何项目。若要回来，需要 {workspace} 的管理者（{managers}）重新邀请你，或由组织所有者添加你。       |
| `danger.lastManagerLeave`                                  | You’re the only Manager of {workspace}. Make someone else a Manager before you leave.                                                                                                                                                                | 你是 {workspace} 唯一的管理者。离开前，请先将其他人设为管理者。                                                                                                                             |
| `workspaces.notAMember` / `…askManagers`                   | Not a member / To work here, ask a Manager of {workspace} to invite you — {managers}.                                                                                                                                                                | 非成员 / 如需在此工作，请联系 {workspace} 的管理者邀请你——{managers}。                                                                                                                      |
| `members.selfDemoteTitle` / `…Body` / `…Keep` / `…Confirm` | Make yourself a {role}? / You won’t be able to change roles in {workspace} any more — including your own. To become a Manager again, ask another Manager of {workspace} ({managers}) or the organization’s Owner. / Keep Manager / Make me a {role}  | 将自己设为{role}？ / 你将无法再更改 {workspace} 中的角色——包括你自己的角色。若要重新成为管理者，请联系 {workspace} 的另一位管理者（{managers}）或组织所有者。 / 保持管理者 / 将我设为{role} |
