...
the **Story-2.5 `/items` List/Tree view** (every project issue, sortable +
...
backlog/sprint" is the existing `/items` navigator reached from `/backlog`, not
...
**Code:** 4.2.3 wires the toolbar link to `/items` (the active project's
...
reusing the `/items` primitives board-scoped). The backlog's equivalent was
...
which is exactly what Motir's shipped `/items` + board filter already are.
...
Every filter affordance is the SAME shipped /items component the board already
...
## The trigger states (identical to /items + board)
...
**carries the active filter query** into `/items` when a filter is applied
(`/items?filter=…`). The backlog design-notes already anticipated this: _"when
