...
| `repos[].repo`      | **no control in this form — _+ Link pull request_, already in the card head, is the door.** A repository enters How to test by having its pull request linked, and leaves by being unlinked | one row per repository with a linked pull request — the item's own before its descendants'       |
...
| MOTIR-5453 | **GIVES** `sections[].source` a drawing (the _from …_ line). **TAKES** two things: that `commitSha` is nullable and the null case is a DISTINCT drawn state (13c), not an empty input; and that **`projectRepos[]` is RETIRED** (decision 8b) — it existed only to populate a picker this form no longer has, so the draft read drops it |
