...
| **The shared APPROVAL CONTROL (8 states)**        | **`approval-control.mock.html`** (HTML mockup)                              | The approval gate's ONE control, drawn once with a panel per state, plus the design-result subject body beside it. Gates MOTIR-4792 and, through it, every consumer. The Approvals TAB (MOTIR-4879) and the pull-request-merge subject body (MOTIR-4882) are NOT here. See below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
## ⭐ The shared APPROVAL CONTROL — one element, eight states (Story MOTIR-4778 · MOTIR-4789 — `approval-control.mock.html`)

**The asset:** `approval-control.mock.html` + `approval-control.png` (1200 CSS px viewport, `deviceScaleFactor: 2` → 2400×5006).

**What it is for.** Motir is about to ask a person to say yes to more than one kind of thing — a published design now, a pull-request merge next, a decision gate after that. The failure this asset exists to prevent is the ordinary one: a green Approve here, an "Accept design" there, a "Merge pull request" somewhere else, each with its own confirm, its own decided state and its own failure copy. Each reads fine alone; together they teach a person that approving means something different in each place. So the control is drawn ONCE, and the subjects differ around it.

**Its authority is `docs/decisions/approval-gates.md`** (merged on `main` at `9840d00ea`). Where this asset and that record disagree, the record wins. It settles the state set (§6b), the routing (§2), the retention that state E renders (§6c) and the reopen lifecycle (§6d).

### Scope — and what is deliberately NOT here

This asset draws **the control** and **the design-result subject body**. It does NOT draw:

| not here                                                                                                                           | owner          |
| ---------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| the Approvals TAB — rows, ordering, empty state, post-decision behaviour                                                           | **MOTIR-4879** |
| the PULL-REQUEST MERGE subject body — branch, base, check state, author                                                            | **MOTIR-4882** |
| the six MERGE refusals (checks not green · conflict · protected branch · no linked PR · App lacks permission · decided by another) | **MOTIR-4882** |

Those moved when MOTIR-4778 was split into one-feature stories. Drawing them here would TAKE scope those cards claim.

### The eight states — the enum IS the checklist

| panel | state                 | what the reader sees                                      | tokens                                                              |
| ----- | --------------------- | --------------------------------------------------------- | ------------------------------------------------------------------- |
| **A** | `awaiting`, yours     | subject + the two verbs — Approve · Request changes       | `--el-tint-yellow` pill, `--el-accent` / `--el-accent-text` primary |
| **B** | `awaiting`, not yours | the state, who it waits on, **no controls**               | same pill, no `.btn`                                                |
| **C** | confirming            | what pressing Approve is about to DO, in words            | `--el-tint-lavender` confirm block                                  |
| **D** | in flight             | pending treatment while the decision is recorded          | `--el-tint-sky` pill, disabled `.btn`                               |
| **E** | `approved`            | who · when · the version · what it moved · **Files kept** | `--el-tint-mint` pill + `.pinned` chip                              |
| **F** | `changes_requested`   | the note foregrounded, and what happens next              | `--el-tint-peach` pill, `--el-warning` note rule                    |
| **G** | `superseded`          | the question was **withdrawn** — nobody decided it        | `--el-muted` fill, `--el-text-secondary` ink                        |
| **H** | refused               | why the decision could not be carried out, **in place**   | `--el-danger-on-surface` ink                                        |

**Three of these are load-bearing and easy to get wrong:**

1. **G · `superseded` is deliberately COLOURLESS.** It is written by the PRODUCT, not by a person — a newer design was published, so the question was withdrawn. It carries no actor, no verbs and no note, and it must not read as a decision somebody made. The ADR's audit framing depends on that distinction: a withdrawn question read as a human decision is a false entry in the loop evidence. So it takes `--el-muted` and the neutral "Withdrawn" pill, never a decided tone.

2. **E · approved says "Files kept."** ADR §6c pins an approved version's assets so a later supersede cannot feed them to the orphan-GC. That is a fact the reader can act on — it is why their approval stays checkable — so it is drawn, not left as an implementation detail.

3. **C · the confirm step says what approving DOES**, in the reader's own terms, as a list: it records the decision, it keeps this version's files, and it moves the work item on and starts the items waiting on it. A confirm step that only asks twice adds friction without adding information.

**The confirm is an INLINE two-step, not a modal.** The control already sits inside a section the reader is looking at; a modal would take the subject off screen at exactly the moment they want to check it once more. The same gesture serves both gate kinds — the merge kind adds one line of consequence rather than a different shape.

### Placement, and the one layout rule

The control renders INSIDE the section that owns its subject — never a page of its own. On the item page that is the `Design result` section, after Development (panel 0). **The control sits at the HEAD of the section, never below the artefact**, so a reader facing a 32rem mock frame is not scrolling to find out whether a decision is owed of them.

### What it COMPOSES rather than redraws

- **`design-result.mock.html`** — the note, the sandboxed mock frame at its 32rem height, the screenshot and the superseded treatment are all specified there. This asset cites it and draws only what the gate ADDS around it (panel S).
- **`acceptance-panel.mock.html`** — the shipped approval gesture this generalises. **Carried:** the verb pair, the decided read-only variant, the `pending | approved | changes_requested` vocabulary, the provenance strip. **Deliberately different:** the acceptance receipt FREEZES on approval and refuses to be superseded; a design gate PINS instead, because a design legitimately evolves after approval and the revise loop depends on republishing (ADR §6c). The asset must not be read as a licence to copy the freeze.

### Primitives and tokens

`ContentSectionCard` (`.card`), `Button` variants (`.btn-primary` / `.btn-secondary` / `.btn-quiet`), `Pill` (tint background + `--el-text-strong`, finding #35), and the avatar/meta grammar the acceptance panel already uses. **No new visual primitive is invented.**

Every colour is Tier-3 `--el-*`; every surface's radius, padding, height and elevation is an element-semantic shape token (`--radius-card` / `--radius-input` / `--radius-badge` / `--radius-btn`, `--spacing-card-padding` / `--spacing-chip-*` / `--spacing-btn-x-sm`, `--height-btn-sm`, `--shadow-subtle`). No Tier-0 `--color-*` and no raw `rounded-*` / `p-*` / `h-*` outside the copied token block.

**Ink:** captions on tinted surfaces take `--el-text-secondary` (6.18–6.80:1 on all four surfaces, both themes), never `--el-text-muted` (4.12–4.34:1 off-white). `--el-text-faint` is not painted at all. The refusal's ink is **`--el-danger-on-surface`** — `--el-danger-text` appears nowhere, because it is the ink FOR a danger fill and renders white-on-white on a page. Monospace work-item keys take `--el-text-identifier`, which this mock declares in its own token block (it is newer than the block it was lifted from).

### Planning flags

- **The `superseded` panel is new since the card was authored.** It comes from ADR §6b, which the card's own state table predates. MOTIR-4792 must build seven visual states plus the not-yours variant, not six.
- **State E's "Files kept" implies a prop MOTIR-4792 does not yet have** — whether this version's assets are pinned. It is a DTO field, and MOTIR-4788's audit columns are where it comes from.
- **`--el-danger-surface` is referenced with a `--el-tint-rose` fallback** in the refusal rule, because not every palette in the block declares it. If the token is universal, drop the fallback when this is implemented.

### GIVES / TAKES

| work item                                  | GIVES / TAKES | what                                                                                                                                                                                                                                                                               |
| ------------------------------------------ | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MOTIR-4792** (the control)               | **GIVES**     | the full state set, the inline confirm decision, the head-of-section placement rule, and the token/primitive map. **TAKES nothing** — its criteria already say "the control"; this asset only makes the state count seven-plus-one rather than six, which is amended on that card. |
| **MOTIR-4879** (the Approvals tab)         | **GIVES**     | the row's composed control, so the tab does not re-specify decided/awaiting treatments. **TAKES nothing** — its own rows, ordering and empty state are explicitly left to it.                                                                                                      |
| **MOTIR-4882** (the merge gate)            | **GIVES**     | the confirm grammar and the refusal placement (in place, never a toast). **TAKES nothing** — its subject body and its six refusals are explicitly left to it.                                                                                                                      |
| **MOTIR-693** (9.2 hosted review)          | **GIVES**     | the control its review UI composes instead of drawing its own, per ADR §5.                                                                                                                                                                                                         |
| **MOTIR-2664** (`design-result.mock.html`) | **neither**   | `done`; composed and cited, not redrawn or re-opened.                                                                                                                                                                                                                              |
