...
| **The PENDING-PLAN indicator on the item page**   | **`pending-plan-indicator.mock.html`** + `pending-plan-indicator.png`       | What the item page says about a plan that proposes to change or ARCHIVE this card — the page reads no plan today, so this is the NONE-exists case. COMPOSES `ArchivedNotice` (the slot and the geometry), the shipped AI-proposal grammar (`ExpansionNudgeBanner`'s lavender `Card` + `Sparkles`), `detail-arrival`'s three-tier allocation and `WorkItemPlanEntrance`; none is redrawn. Settles the SLOT (first in `<main>`, above Description) and its TIER (**tier 2, inside the page's existing concurrent group** — not the late stack, which would resize a top-of-page box), the TONE with the archived mold rejected and every ink MEASURED, **one row per `PlanStatus` value including `stale`** (which MOTIR-4197's boundary never named), the copy per op and per plurality with its `zh` obligation, the three ABSENCES as panels, and the READ with its repository method and the reverse index that serves it. Fifteen panels, light + dark + 390. Design card MOTIR-4256. Gates MOTIR-4197. See below. |
