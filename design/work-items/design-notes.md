...
**The asset:** `approval-cta.mock.html` + `approval-cta.png` (1328 CSS px viewport — a 1280 page plus the board's gutters,
`deviceScaleFactor: 2`). It gates **MOTIR-5229** (the band and its one door), which carries
...
The shipped `DesignResultSection` inside the shipped `ContentSectionCard` was rendered through
...
| **MOTIR-5488** (open, PR #2894)                                  | Nothing.                                                                                                                                                                    | Nothing drawn here depends on it; planning flag 3 names the overlap.                                                                                                                                                                |
...
3. **Open PR #2894 (MOTIR-5488) changes both neighbours of this band, not the band itself.** A design
   card with an open linked pull request loses its standalone Design result section and is decided
   by the merge gate, so a `design_result` gate, and this band, occur only on a design card with no
   pull request. The band is unaffected. Two things follow: panel 4's port contents follow whatever
   `DesignResultPanel` ships, and whichever of the two PRs merges second reconciles
   `design-notes.md`, where both add a section. Flag 2 is the Development-section half of the same
   split.
