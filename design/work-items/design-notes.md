...
| **The item page hands the decision over — the call-to-action band** | **`approval-cta.mock.html`**                                                          | While the Design result section's gate is `awaiting` and the reader may decide it, the section holds band 1 of the frame plus ONE control, **Review & approve**, into the full-screen approval overlay; the look-only (`B`), approved (`E`), changes-requested (`F`), withdrawn (`G`) and no-gate cases render as today. Narrow in `zh`. AMENDS the frame's item-page placement (dated 2026-09-15). MOTIR-5228 (story MOTIR-5215) — gates MOTIR-5229. See § _The item page HANDS THE DECISION OVER_.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
**The asset:** `approval-cta.mock.html` — the two-file set (`docs/decisions/design-result.md` AMENDMENT 4: no `.png` export). It gates **MOTIR-5229** (the band and its one door), which carries
it in `blocked_by`; **MOTIR-5230** and **MOTIR-5231** assert against it.

**What it draws:** the Design result section once deciding has moved off the item page. A
decision that is waiting on a reader who may make it becomes a **call-to-action band**: what is
waiting, which version, how long it has waited, and ONE control, **Review & approve**. That
control opens the approval overlay ([MOTIR-5214](motir:cmtxm4v3600edhztx2s78ff0u),
`design/workbench/design-notes.md` § 22) over the page. Every other state renders exactly as it
does today.

**Panels:** 1 the band in place on `/items/[key]` at 1280, after Development, measured ·
2 anatomy, for the reader it is routed to and for an admin it is not routed to ·
3 the three states the page KEEPS — `B` see but not decide, `E` approved, `F` changes requested,
`G` withdrawn · 4 no gate at all — the shipped `DesignResultPanel`, unchanged · 5 narrow
(`< md`), in `zh`.

### Rendered first

The shipped `DesignResultSection` inside the shipped `ContentSectionCard` was rendered — on `origin/main` after MOTIR-5488 merged (`3e1cf0e30`), which reshaped the port — through
the repo's vitest + RTL setup (`renderWithIntl`, the real `messages/en.json` and
`messages/zh.json`) in eight states: awaiting with and without `canDecide`, awaiting with an
unresolved routing, approved, changes requested, superseded, no gate with a result, and no gate
on an empty design card. Every class below — the section card, the frame chrome, band 1's
`FrameHeader`, band 3's row, the record strip, the dead port, the empty state — is copied from
that render's emitted markup. Two facts from it shape the design:

- **Band 1 already says what the band needs to say.** `FrameHeader` renders the kind label, the
  subject version and the state pill, in one wrapping row. The band keeps that row
  **byte-identical** and adds one thing to its meta line: how long ago the question was asked.
- **State `B` NAMES the approver on `origin/main`.** MOTIR-5191 (`9421599ae`, 2026-09-12)
  resolves `routedToLabel` in `approvalGatesService.getForWorkItem`, and both the item page and
  the overlay pass it. The frame renders _Waiting on Ada Lovelace._ The generic _this work item's
  assignee_ fallback appears only when routing resolves to nobody. The card was written 12 minutes
  before that merge, and its premise that the line is generic is out of date. The finding is
  recorded on the card.

### ⚠️ The rule, in one sentence — the code card is closed against it

**The band replaces the frame ONLY when the gate is `awaiting` AND the reader `canDecide`; every
other state, and a card with no gate, renders exactly as it does today.**
...
judgement. `canDecide` is the section's existing prop, unchanged.

| gate on the card                            | `canDecide` | the section renders                                                       | panel |
| ------------------------------------------- | ----------- | ------------------------------------------------------------------------- | ----- |
| **`awaiting`**                              | **true**    | **the call-to-action band** — band 1 + one control, no port, no verbs     | 1 · 2 |
| `awaiting`                                  | false       | the frame in state `B` — port shown, no verbs, _Waiting on {name}._       | 3     |
| `approved`                                  | either      | the frame in state `E` — the pinned version and _Files kept_ / _not kept_ | 3     |
| `changes_requested`                         | either      | the frame in state `F` — the version sent back, _the agent will publish…_ | 3     |
| `superseded`                                | either      | the frame in state `G` — dead port, colourless, no verb                   | 3     |
...
`DesignResultPanel` reports `failed` whenever it has no mock (`useReportPortRenderStatus(… ? 'rendered' :
'failed')`), and `ApprovalGateControl` shows `PortFailedAlert` on `portFailed && !withdrawn`, which does not
exclude a decided gate. That is a defect, filed as its own bug. The asset draws `F` without the alert.

**`C` confirming, `D` in flight and `H` refused no longer occur on the item page.** They exist
only between a press and its answer, and the only press left is in the overlay, which renders
them there. **`X` (the port failed)** is unreachable on the page for the same reason: the band
mounts no port.

### Anatomy — the band

The band is **the frame with its bands 2 and 3 replaced by one row**. Nothing new is invented:
the container is the frame's own inline chrome, band 1 is `FrameHeader`, and the row is band 3's
own row recipe.

| #   | element                        | composes (as shipped)                                                                           | colour                                                     | shape                                                   |
| --- | ------------------------------ | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------- |
| —   | the section                    | `ContentSectionCard` — _Design result — what this work item produced_ (unchanged)               | `--el-card` on `--el-border`, `--shadow-card`              | `--radius-card`, `--spacing-card-padding`               |
| —   | the band's container           | the frame's inline chrome: `overflow-hidden rounded-(--radius-card) border`                     | `--el-border`                                              | `--radius-card`                                         |
| 1   | kind                           | `FrameHeader` — `text-sm font-semibold`                                                         | `--el-text`                                                | —                                                       |
| 2   | subject version                | `FrameHeader` meta — `approvalGate.designResult.meta.withVersion`, `subjectVersion` sliced to 8 | `--el-text-secondary` (the hash is plain text, as shipped) | —                                                       |
| 3   | waited for (**NEW**)           | the same meta span, `·` then `approvalGate.cta.asked` over `relativeTime(gate.createdAt)`       | `--el-text-secondary`                                      | —                                                       |
| 4   | state                          | `Pill tone="awaiting"` — _Awaiting you_ (unchanged)                                             | `--el-tint-yellow` + `--el-text-strong`                    | `--radius-badge`, `--spacing-chip-x` / `-y`             |
| 5   | the sentence (**NEW**)         | band 3's left-hand line — `text-[13px]`                                                         | `--el-text-secondary`; a named person in `--el-text`       | —                                                       |
...
**The row** is band 3's `flex flex-wrap items-center gap-x-3 gap-y-2 border-t
border-(--el-border-soft) px-4 py-3`, with the sentence on the left and the control pushed right
(`ml-auto`). Band 1 keeps its `border-b`, so the row sits directly under it and there is no empty
port between them.

**The meta line reads three fields, all on `ApprovalGateDTO` (`lib/dto/approvalGate.ts`), and no
others:** `kind` (through the kind's label, as band 1 already does), `subjectVersion`, and
`createdAt` as _asked {when}_. It promises no dependent count, no file list and no author, because
no read on the page produces them. `createdAt` is when the gate was raised, which is when the
question was first asked. A republish supersedes the old gate and raises a new one, so the clock
restarts with the version, which is correct.

**The sentence has two forms, keyed on routing, never on authority.** `routedToId` is on the DTO
and the page knows the viewer:

- **routed to the viewer** — _Review the design full screen, then approve or request changes._
- **routed to someone else, and the viewer may still decide it** (the reporter when there is an
  assignee, or any admin: `docs/decisions/approval-gates.md` §2 as amended by MOTIR-4911) —
  _Waiting on **{name}** — you can decide it too. Review it full screen first._ `{name}` is
  `routedToLabel`. When it is null, the first form renders instead, because a sentence naming
  nobody says nothing.

**The pill stays _Awaiting you_ in both**, as the frame already does for any reader who may decide.
The band is the same band and the door is the same door, which is the story's criterion that the
door serves AUTHORITY and not routing.

### The control, and its string — SETTLED

**`Review & approve` · `审阅并批准`.** The register is recorded so it is not re-opened by accident.
_To approve_ is the To-approve tab's name, a noun phrase for a QUEUE, and that is right for a tab.
A button is a verb a person presses, and a label that reads like a category leaves the reader
unsure whether pressing it approves something straight away. _Review & approve_ says what happens
next: you look first. That is the ordering the frame's own design was corrected into
(§ _The UNIVERSAL APPROVAL FRAME_, second cut).

**It is ONE string and ONE catalogue entry**, shared with the status control's door
(§ _The status control says so_, MOTIR-5523). That asset already draws these exact words and
committed to following this one. Its code card (MOTIR-5528, PR #2897) adds the entry as
`approvalGate.statusHeld.reviewAndApprove`, and the band reads that same key. If MOTIR-5229 lands
first, it adds the entry under that name with these two strings, and never a second key for the
same words.
...
`lib/approvals/overlayAddress.ts`, written with `shallowPush`. The overlay opens over the item
page, Close strips the two parameters, and the reader is back on the card at the same scroll.
Nothing is unmounted. **The glyph is `ScanEye`**, the status control's own, so the two doors into
one decision look like one door.

### The ACCESS PATH — where a reader meets it

On `/items/[key]`, in the late stack (`LateSections.tsx`), **the Design result section's own slot,
after Development**, where the frame sits today. Panel 1 draws it in place at 1280 with the
page's real `grid-cols-1 gap-6 md:grid-cols-[1fr_18rem]`. **The band does not move and adds no
navigation.** Two other doors lead to the same overlay, and neither is drawn here: the To-approve
row (§ 22 Panel 9) and the status control's held line (§ _The status control says so_).

### Measured (Chromium, the mock, 1× scale)

The band is the section card's full inner width, so it inherits the page's grid rather than setting a width of its own. Nothing overflows: the document is exactly the board width at both sizes.

| viewport            | main column | band (w × h)  | band 1                      | the row                         | control (w × h)              |
| ------------------- | ----------- | ------------- | --------------------------- | ------------------------------- | ---------------------------- |
| 1280 (panel 1)      | **918**     | **868 × 107** | 866 × 49                    | 866 × 56                        | 166 × 32 (`--height-btn-sm`) |
| 390 (panel 5, `zh`) | 358         | 322 × 164     | wraps, pill on its own line | stacked: sentence, then control | 117 × 32, left-aligned       |

### Narrow (`< md`)

Panel 5, in `zh`. Band 1 already wraps, and its pill drops under the meta line when the row is
too narrow. **The row stacks:** the sentence takes the full width and the control drops to its own
line, left-aligned (`md:ml-auto` instead of `ml-auto`), at its intrinsic width. It is not stretched:
a full-width primary button on a phone reads as the page's main action, and this is one section of
many.
...
`{when}` is `useFormatter().relativeTime(gate.createdAt)` — _2 days ago_ / _2天前_. Every other
string in the band is already in the catalogue: `approvalGate.designResult.kindLabel`,
`.meta.withVersion`, `approvalGate.state.awaitingYou`, and `designResult.title` / `.gloss` for the
section card.

### What the band is NOT

- **Not a second frame.** It mounts no port and no verbs, and `ApprovalGateControl` is not
  modified. The band is a sibling rendering in `DesignResultSection` that reuses `FrameHeader`'s
  markup. `tests/approval-gate-one-language.test.ts` stays green without an edit.
- **Not the pending indicator** ([MOTIR-4908](motir:cmtt4ogn7000fhutx7zrhizuo)) on the page
  header, the board card or the list row.
- **Not a redraw of the frame** (`approval-control.mock.html` still owns it and its nine states), of
  the port's contents (`design-result.mock.html`) or of the overlay (§ 22).

### ⚠️ AMENDMENT — 2026-09-15 (MOTIR-5228), to the frame's placement on the item page

**Superseded, for one case only.** § _The UNIVERSAL APPROVAL FRAME_ → _Placement_ records, for
`/items/[key]`: _"it **IS** the `Design result` section — it REPLACES today's read-only
`DesignResultPanel` in the late stack (`LateSections.tsx`), same slot, same position after
Development"_. `approval-control.mock.html` panel 0a draws the same thing (_"the frame IS this
section"_), and `DesignResultSection.tsx`'s header quotes it back.

**What replaces it:** when the section's gate is **`awaiting` and the reader `canDecide`**, the
section holds the **call-to-action band** from this section, not the frame, and the frame renders
in the approval overlay (§ 22 of the workbench notes) that the band's one control opens.

**What it does NOT replace:** the slot, its position after Development, and the section card are
unchanged. In every other state the frame still IS this section: `B`, `E`, `F`, `G`. So is the
no-gate `DesignResultPanel`. **`approval-control.mock.html` still owns the frame** and all nine of
its states. Neither that asset nor its section is edited, and they remain the record of the frame
as it was first placed. The Workbench home it names was superseded separately, by § 20's
amendment.

**Why:** the item page is the only door to a decision for everyone the To-approve tab does not
route it to: a reporter with an assignee present, and every admin (routing vs authority,
`docs/decisions/approval-gates.md` §2 as amended by MOTIR-4911). A decision that has a full-screen
surface cannot offer those people the smaller one, and leaving the verbs beside a door would teach
two ways to approve.

### GIVES / TAKES

Swept over MOTIR-5215's whole subtree (MOTIR-5228 · 5229 · 5230 · 5231) and every key this section
and the mock name, on the ELEMENT, STRUCTURE and PREMISE axes. `git grep` over `design/` for the
subtree's keys finds two referrers besides this section: § _The status control says so_ (names
MOTIR-5228 for the string, settled above to the same words) and the workbench notes' § 22 (names
MOTIR-5215 as the item page's door). Both agree with it.

| work item                                                        | GIVES                                                                                                                                                                       | TAKES                                                                                                                                                                                                                                                                                                            |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MOTIR-5215** (the story)                                       | The design half of its criteria: the band and its meta line, the one control and its string in both locales, the three kept states, the no-gate case, the dated amendment.  | **PREMISE:** _"routedToLabel={null} … waiting on the assignee"_ is out of date — MOTIR-5191. Nothing else.                                                                                                                                                                                                       |
| **MOTIR-5229** (the band and its one door)                       | **ELEMENT:** the band (panels 1–2, 5), the sentence's two forms, the control and its address, the copy table. **STRUCTURE:** the one-sentence rule and the state checklist. | **ELEMENT:** the verbs, the confirm band and the port on the awaiting-and-decidable path, which it removes. **PREMISE:** planning flag 1 — the repaint needs a signal the section and the rail can reach.                                                                                                        |
| **MOTIR-5230** (the vitest gate)                                 | The seams to assert: band iff `awaiting ∧ canDecide`; no approve control in the section's tree while awaiting; each kept state still renders; the no-gate path untouched.   | Nothing.                                                                                                                                                                                                                                                                                                         |
...
| **MOTIR-4789** (the frame's design) · `done`                     | Nothing.                                                                                                                                                                    | **PREMISE:** _"the frame IS this section"_ on `/items/[key]`, now true for every state except `awaiting ∧ canDecide`, by the dated amendment above. Its asset and nine states are unchanged.                                                                                                                     |
| **MOTIR-4792** (the frame, and the section becoming it) · `done` | Nothing.                                                                                                                                                                    | **STRUCTURE:** the section as the frame for the awaiting-and-decidable case. Superseded going forward by the amendment above, not amended: a `done` card is history, and it shipped correct against the design current at the time.                                                                              |
| **MOTIR-5222 / MOTIR-5224** (the overlay's design, its host)     | Nothing.                                                                                                                                                                    | **ELEMENT:** the destination — the overlay, its address (`lib/approvals/overlayAddress.ts`) and its one close — composed, never drawn.                                                                                                                                                                           |
...
| **MOTIR-5488** (merged, PR #2894 `3e1cf0e30`)                    | Nothing.                                                                                                                                                                    | **PREMISE:** a `design_result` gate exists only on a design card with NO open linked pull request (AMENDMENT 4 Q8), so this band never renders on a card whose result lives in the Development block. **ELEMENT:** panel 4 and the kept ports draw its shipped panel — mock frame first, the note one link away. |

### ⚠️ Planning flags — surfaced by this pass

1. **A decision made in the overlay has to reach two surfaces on the item page that
   `router.refresh()` alone does not reliably reach. MOTIR-5229's repaint criterion owns both, and
   neither mechanism exists yet.**
   - **The section.** `DesignResultSection` seeds `const [current, setCurrent] = useState(gate)`,
     so a server render carrying the decided gate does not reset it. The band must key on the
     SERVER `gate` prop, or watch `useDecidedGateState(gate.id)` (`lib/approvals/decidedGates.ts`,
     which the overlay already announces to). Otherwise it keeps offering _Review & approve_ over a
     decided question.
   - **The status rail.** Its in-browser path (MOTIR-5212) is `useOptimisticStatusWriter`, applied
     from the decide response's `outcomeRef`. The overlay is mounted in the authed shell, outside
     `OptimisticStatusProvider`, and `announceGateDecided` carries the state and not the `outcomeRef`.
     So from the overlay only the refresh reaches the rail, and MOTIR-5212 measured that apply being
     intermittently lost. **Carry `outcomeRef` on the announcement and apply it from the provider**,
     or an equivalent signal. Record the choice in MOTIR-5229, not in this asset.
2. **The Development section's merge frame is a second place the item page could submit a
   decision, and no card in this story owns it.** `DevelopmentSectionBody` wraps its rows in
   `DevelopmentGateFrame` while a `pull_request_merge` gate awaits (design § 20, MOTIR-5336), and
   [MOTIR-5484](motir:cmu1aj1bj00gkhyoi0iuds6xy) (`blocked`) plans to give that frame its verbs —
   _Approve and merge · Request changes_ — on this page. That contradicts this story's rule that no
   `GateDecision` is submitted from the item page. **Re-scope MOTIR-5484** so the Development section
   takes this same band, with `kind: 'pull_request_merge'` and the overlay MOTIR-5437 builds for it,
   and puts the verbs in the overlay only.
3. **PR #2894 (MOTIR-5488), which merged during this pass as `3e1cf0e30`, narrows where the band can
   occur, and leaves the band itself unchanged.** A design card with an open linked pull request no longer has a
   standalone Design result section. Its result renders inside the Development block, and the merge gate
   decides it (`docs/decisions/design-result.md` AMENDMENT 4 Q8). So a `design_result` gate, and this
   band, occur only on a design card with NO pull request. The kept states' ports and panel 4 are drawn
   with that PR's shipped panel: the mock frame first, the note one link away. Flag 2 is the Development
   block's half of the same split.
