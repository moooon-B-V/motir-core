...
**The asset:** `approval-control.mock.html` + `approval-control.png` (1200 CSS px viewport, `deviceScaleFactor: 2` → 2400×7440).
...
2. **`E` keeps showing the approved version** even after a newer design is published — which is what ADR §6c's pin is FOR. Without the port, `E` is a row of metadata about something nobody can look at any more, and the pin's purpose is invisible.
