-- HOW TO TEST records (Story MOTIR-4906 · Subtask MOTIR-5328). In ONE atomic,
-- ADDITIVE step (table + indexes + FKs + the partial unique index + RLS land
-- together, so there is never an unguarded window, and nothing existing changes
-- — it ships in one deploy with no expand/contract sequence).
--
-- RLS shape = a PURE active-workspace gate, identical to `design_evidence`
-- (20260811145123): every row carries a NON-NULL `workspace_id` and every write
-- happens inside an active workspace context (`testInstructionsService.publish`
...
-- missing_ok=true means an unset GUC → NULL → row hidden (safe failure).
-- Uncorrelated by construction: the predicate names no other table, so the
-- per-row cost is one comparison (the RLS per-row-tax lesson).
--
...
-- CreateIndex
CREATE INDEX "test_instructions_work_item_id_repo_id_commit_sha_created_a_idx" ON "test_instructions"("work_item_id", "repo_id", "commit_sha", "created_at" DESC);
...
-- AddForeignKey
ALTER TABLE "test_instructions" ADD CONSTRAINT "test_instructions_repo_id_fkey" FOREIGN KEY ("repo_id") REFERENCES "github_repo"("id") ON DELETE CASCADE ON UPDATE CASCADE;
...
-- The load-bearing invariant: AT MOST ONE current record per (work item,
-- repository). A partial unique index (only current rows participate) is the
-- DB-level backstop behind the service's row lock, so a publish race can never
-- leave two current rows. History rows (is_current=false) are unconstrained.
--
-- ⚠️ Its column list (work_item_id, repo_id) is deliberately NOT the column list
-- of any `@@index` on this model — Prisma's differ pairs a DB index to a
-- datamodel index BY COLUMN LIST and cannot express a WHERE clause, so a
-- collision would surface as a permanent spurious RENAME (MOTIR-1960).
CREATE UNIQUE INDEX "test_instructions_one_current_per_item_repo"
  ON "test_instructions" ("work_item_id", "repo_id")
  WHERE "is_current";
