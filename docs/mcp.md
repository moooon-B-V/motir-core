...
| Scope                | Gates                                                                                                                                                                                                                             |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `read`               | `get_work_item`, `list_ready`, `next_ready`, `dispatch_prompt`, `search_work_items`, `whoami`, `list_sprints`, `validate_sprint`, `validate_work_item`, `get_plan_status`, `open_plan_session`                                    |
...
`initialize` handshake and registers **35 tools**.
...
- **`submit_plan_session`** — send the accumulated intent as ONE job.
  **Output** — `structuredContent`: `{ jobId, planId, session }`. It **submits
  ...
  outcome. A single-turn thread is sent verbatim; a multi-turn thread is sent as
