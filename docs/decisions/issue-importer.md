...
| **status**         | project `workflow_status` | Map source status → one of the **target project's** `WorkflowStatus` keys. Unmatched → **user choice in the wizard: map to an existing status or a chosen default — never silently dropped.**                                                                                                            | `status` is NOT settable on create (create lands in `isInitial`) or via `updateWorkItem`. The mapped status is applied through the workflow transition path — see Consequences → _required extension_.                                                  |
