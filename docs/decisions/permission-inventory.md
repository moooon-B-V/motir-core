...
| `/api/work-items/approval-gate`                         | GET         | `workItemsService.getWorkItemByIdentifier` → `assertCanBrowse`; then `approvalGatesService.getForWorkItem` (its `canDecide` asks `approval:decide_any`) and `designEvidenceService.getForGateSubject` | `project:browse`        | existing | R2  |
