...
**On top of the floor, never instead of it, authority is `assignee OR reporter OR admin`** (`docs/decisions/approval-gates.md` §2's MOTIR-4911 amendment; Yue, 2026-09-08). An earlier draft settled it on two catalog keys and was withdrawn: authority here follows a RELATIONSHIP to the work item, with an org-wide override for a workspace owner/admin (`isWorkspaceManager`), because the failure to prevent is a gate whose single recipient has left and nobody able to unblock the work.

⚠️ **ROUTING and AUTHORITY are different axes, and a reader of this row should not collapse them.** A gate is SHOWN to exactly one person (`assigneeId ?? reporterId`) and may be PRESSED by three. That is coherent rather than sloppy: routing answers _whose job is it to look?_, where a gate shown to two people is a decision neither owns; authority answers _may this press be honoured?_, where widening costs the queue nothing because a reporter deciding from the item page never sees the gate in their own tab.
...
| Operation                                               | Verbs       | Gate today                                                                                      | Permission              | Decision | Why |
| ------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------- | ----------------------- | -------- | --- |
| `/api/approval-gates/[id]/decide`                       | POST        | `approvalGatesService.decide` → `assertPermission` (the kind's floor) + assignee/reporter/admin | `work_item:edit`        | new      | R64 |
