...
**On top of the floor, never instead of it, authority is `assignee OR (reporter AND the item has NO assignee) OR admin`** (`docs/decisions/approval-gates.md` §2's SECOND amendment, MOTIR-5192; Yue, 2026-09-11). Authority follows a RELATIONSHIP to the work item, with an org-wide override for a workspace owner/admin (`isWorkspaceManager`) — never a catalog key, which an earlier draft settled it on and which was withdrawn. It is resolved in exactly one place, `approvalGatesService.resolveGateAuthority`, called by the decide door and by the render read so the two cannot disagree.
...
| Operation                                               | Verbs       | Gate today                                                                                                        | Permission              | Decision | Why |
| ------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------- | ----------------------- | -------- | --- |
| `/api/approval-gates/[id]/decide`                       | POST        | `approvalGatesService.decide` → `assertPermission` (the kind's floor) + assignee / reporter-if-unassigned / admin | `work_item:edit`        | new      | R64 |
