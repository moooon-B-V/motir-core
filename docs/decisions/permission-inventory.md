...
**R64.** The APPROVAL-GATE DECIDE DOOR (Story MOTIR-4778 · MOTIR-4790) — _record a decision on one approval gate, whatever its kind._ Governed by a **permission FLOOR plus a RELATIONSHIP test**, and it is the first row in this table whose answer is not a permission alone, so the split is worth stating rather than inferring from the code.

The floor is the KIND's own key, supplied by its `GateHandler` (`lib/approvalGates/registry.ts`) — **`work_item:edit`** for `design_result`, on R52's argument unchanged: deciding evidence attached to an item is editing that item, and the project is resolved from the ITEM rather than from the actor's active project. A non-browser is refused as `ProjectNotFoundError` (404) and a browser without the key as `PermissionDeniedError` (403), so neither refusal leaks the other.

**On top of the floor, never instead of it, authority is `assignee OR (reporter AND the item has NO assignee) OR approval:decide_any`** (`docs/decisions/approval-gates.md` §2's SECOND amendment, MOTIR-5192, and THIRD, MOTIR-5292). Authority follows a RELATIONSHIP to the work item for your OWN gate, and the **`approval:decide_any`** key for anyone's — the `attachment:delete_any` shape. Workspace owners/admins hold it through the always-pass rail, the built-in project Admin set holds it, and a custom role can be granted it. **Until MOTIR-5292 that third arm was the workspace ROLE itself** (`isWorkspaceManager`, asked through `projectAccessService.isWorkspaceManagerFor`), which no custom role could carry and which refused a project Admin whose workspace role is `member`; the method is deleted. The key is ungrantable to an API token by derivation — the route is session-authed. It is resolved in exactly one place, `approvalGatesService.resolveGateAuthority`, called by the decide door and by the render read so the two cannot disagree.

> **SUPERSEDED, kept because it was argued and because knowing it was considered matters.** Until 2026-09-11 the reporter arm was UNCONDITIONAL — `assignee OR reporter OR admin` (§2's MOTIR-4911 amendment; Yue, 2026-09-08) — on the reasoning that ~~"the failure to prevent is a gate whose single recipient has left and nobody able to unblock the work."~~ That failure is real and is now answered by the ADMIN arm alone, which is a better-shaped override: a role is visible in the permission grid, grantable to a custom role and auditable, none of which a relationship to a row can be. What the narrowing buys is that an approval belongs to exactly one person — with an assignee on the item and two people able to press, the decision is owned by neither in particular.

⚠️ **ROUTING and AUTHORITY are different axes, and they now COINCIDE for the relationship arms.** A gate is SHOWN to exactly one person (`assigneeId ?? reporterId`), and that is term for term the person the two relationship arms authorise: the gate is pressed by the person it is shown to, or by an ADMIN. So do not collapse the axes back into one query — they part company at the admin override, and only there — and do not read the coincidence as routing having become the rule: an admin deciding from the item page never sees the gate in their own tab.
...
**R65.** The APPROVAL-GATE SWITCH door (Bug MOTIR-4925 · MOTIR-5170) — _read or change WHICH approval gates this project raises._ **R64's neighbour and not its duplicate: R64 governs PRESSING a gate, this governs whether the gate EXISTS at all**, so the two answer different questions about the same object and take different keys.

The key is **`workflow:manage`**, on BOTH verbs, asserted in `approvalGateSettingsService` rather than in the route — the route is reachable by URL whether or not the settings rail offers its row, which is the whole reason the destination guard exists one layer up. `workflow:manage` is the shipped key for _may you configure how work moves through this project_, and a status graph and an approval gate are the two things that decide when work may move; a dedicated `gate:manage` is a reasonable later split and is deliberately NOT made here, because this card has no warrant to grow the catalog.

⚠️ **The WRITE authority is the DESTINATION's, not the ORIGIN's** (`docs/decisions/organization-tier.md` §6). The acceptance-video switch this room's first row controls was an `Organization` column written behind `assertOrgAdmin`; moving the surface to the project tier does not carry that gate along with it, and §6 forbids a relocation that removes a capability — so the destination's gate is what must change.
...
**R67.** The BUG DESTINATION door (Story MOTIR-4927 · MOTIR-4938) — _change which folder, or the project root, Motir files the bugs it creates on its own into._ **`project:administer`**, asserted in `bugDestinationService.setDestination` and, for the room's server-side read, `getSettings`. No `bug:*` key exists, and a filing destination belongs to none of the existing domains, which is `project:administer`'s own registry evidence (`design/projects/design-notes.md` § Bugs §4); `work_item:triage` is scoped to the triage QUEUE and `workflow:manage` to how work MOVES. A dedicated key is a reasonable later split and is not made here. The route answers a NON-browser `404` before any key is consulted, names another project's folder `FOLDER_NOT_FOUND` (`404`, no existence leak), and refuses a browser without the key with `NOT_PROJECT_ADMIN` (`403`). There is no GET route: the Bugs room's page reads the value server-side.

**R68.** The APPROVALS ROOM's RECORDS READ (Story MOTIR-5299 · MOTIR-5305 names the key, MOTIR-5301 consults it) — _list the approval records of one project: pending first, then decided._ **R64's shape for READING rather than pressing: a FLOOR, two RELATIONSHIP arms, and one key that WIDENS.** The floor is **`project:browse`** — a record of the project is a thing about the project, and a non-browser is refused as `ProjectNotFoundError` (404) exactly as every project read is. On that floor every reader sees their OWN records, which need no key at all because they are relationships to a specific row: gates **routed to them and still `awaiting`** (the ONE routing predicate, `awaitingRoutedToWhere`, reused rather than copied) and gates **they decided** (`decidedById = me`, which includes `changes_requested` and excludes `superseded`, whose null decider means nobody decided it). **`approval:view_any`** widens the read from the reader's own records to EVERY record of the project, other people's decisions included — the `attachment:delete_any` / `comment:moderate` / `watcher:manage` shape of _your own, or anyone's with a key_.

It is a PERMISSION and never a role: the built-in project `admin` set holds it through `ROLE_GATED_PERMISSIONS`, workspace owners/admins hold it through the always-pass rail, `member` / `viewer` / the implicit workspace-member grant do not, and a custom role can be granted it. **`isWorkspaceManagerFor` is deliberately NOT the instrument** — it is `owner || admin`, a role check whose own docstring says it is not a substitute for a permission, and a room scoped on it could never be opened to a custom role. The read resolves the scope itself from `projectAccessService.getPermissions`, so no caller can ask for the wider view.

**It is ungrantable to an API token by derivation** (`lib/tokens/grant.ts`): a key is grantable because a token-reachable operation asserts it, and the room is a server-rendered page reading the service directly — no `/api/v1` route and no MCP tool asserts this key. **Enforced by MOTIR-5301** (`approvalGatesService.listRecords`), which lands the read and flips the key in the same change. MOTIR-5305 named it `planned` for one commit, while the role screens listed only enforced keys, so no switch rendered for it before a gate consulted it.

---

**R69.** QUEUE AGAIN (Story MOTIR-5461 · MOTIR-5634; `approval-gates.md` §4 THIRD AMENDMENT, decision 5) — _put a pull request the merge queue removed back into the queue while its head is unchanged._ **Two doors behind one route, and each takes the answer its merge mode already has.** In a `manual` project the body names the card's DECIDED approval gate and the press is `pullRequestMergeService.retryApproveAndMergeMember` — the same door as _Retry merge_ (R64's floor, `work_item:edit`, plus R64's relationship test), because it carries out a decision a person already made rather than making one. In an `auto` project there is no gate to reuse, so the press is `requeueAutoMember` → `assertPermission(work_item:edit)`: the key that governs the card's status, since the press moves it `implemented → in_review`. No new key: whoever may approve or edit the card may put its approved code back in the queue.

**R70.** THE WORKBENCH'S LIVE TAIL (Story MOTIR-5238 · MOTIR-5241) — `/api/workbench/stream`, the Server-Sent-Events route that tells a reader which of their five tabs have moved. **It is USER-SCOPED and takes no key of its own, and that is a decision rather than a gap: the route makes no access decision at all, because it has nothing to decide about.** It answers one reader about their own five tabs; there is no addressable object in the request and nothing another reader could ask it for.

**The frame carries no content.** It names which tabs moved and a watermark — no rows, no titles, no ids — so there is nothing in it a reader could not already read. The client re-reads through the tab reads it already has, and each of those makes its own decision exactly as it does on the page: `homeService`'s four tabs and `approvalGatesService`'s queue both resolve the reader's ACTIVE project through `projectAccessService.filterBrowsable`, and a reader who may not browse it resolves to an EMPTY scope. So the watermark this route polls is all zeroes for such a reader, and a key here would be a second copy of a decision the composed reads already make — the shape R2 refuses elsewhere.

**What the route DOES assert is the SESSION, before the stream opens**: `requireCompliantWorkspaceContext` first, then the reader's active project, then the first read — all of it before the `ReadableStream` is constructed, so a session, 2FA-hold or tenancy refusal is a real HTTP status with a JSON body and no frame is ever written. That is the ordering `app/api/ai/plan/generate/[jobId]/stream/route.ts` established, R58's stream mirrors, and this one mirrors in turn.

**The narrower key would also be the wrong shape.** `project:browse` is the key the composed reads assert per tab; asserting it HERE would refuse the connection outright for a reader who may not browse their active project, where the reads answer the no-existence-leak way — an empty result, never an error. One route that 403s about a project and five reads that quietly return nothing about the same project is two answers to one question.
...
| `/api/ai/code-context`                           | GET       | `codeContextService.resolveCodeContextState` → browse-gated in the service (deliberately NOT `ai:configure`: it is the honest state of the planner's INPUTS, not AI configuration — the service and the route both say so)          | `project:browse`     | existing    | R5  |
...
| `/api/planning/substrate`                        | GET       | `onboardingSubstrateService.readOnboardingSubstrate` behind `requireCompliantSession` + the server-resolved active project (a browse-scoped read of what the project already has; it renders no verdict and chooses no destination) | `project:browse`     | existing    | R5  |
| `/api/ai/plan/route-onboarding`                  | POST      | `aiGenerationService.startRoutingRun` → `assertPermission` (the routing run is a planning turn — it spends the workspace's AI credit and reads the project)                                                                         | `ai:plan`            | new         | R5  |
...
| `/api/%5Ftest/monitors/poll`                 | POST                  | session (`requireContext`); the binding must be the session workspace's (MOTIR-5584)                                            | —          | finding  | R8  |
| `/api/%5Ftest/monitors/enrich`               | POST                  | session (`requireContext`); the bug must be filed by a binding of the session workspace (MOTIR-5853)                            | —          | finding  | R8  |
| `/api/%5Ftest/organizations/erasure-sweep`   | POST                  | session (`requireContext`); runs the organization erasure sweep once in the server process for the E2E lane (MOTIR-6405)        | —          | finding  | R8  |
| `/api/%5Ftest/monitors/resolve-run`          | GET                   | session (`requireContext`); the transition event must be the session workspace's (MOTIR-5709)                                   | —          | finding  | R8  |
| `/api/%5Ftest/monitors/fake`                 | GET, POST             | session (`requireContext`); reads/seeds the SERVER process's fake provider only — no tenant data (MOTIR-5734)                   | —          | finding  | R8  |
| `/api/%5Ftest/monitors/resolve-state`        | GET                   | session (`requireContext`); links read in the session workspace only (MOTIR-5709)                                               | —          | finding  | R8  |
| `/api/%5Ftest/monitors/sync-failure`         | POST                  | session (`requireContext`); the connection must be the session workspace's (MOTIR-5709)                                         | —          | finding  | R8  |
...
| `/api/internal/ai/live-organizations`        | POST                  | serviceAuth                                                                                                                     | —          | no-gate  | R29 |
...
| `/api/internal/ai/plan-revision-reason`      | POST                  | serviceAuth + a LIVE job token (`authenticateAndLimitJobRequest`) — the plan comes from the TOKEN's own job, never the body     | —          | no-gate  | R29 |
...
| `/api/projects/[key]/members`                              | POST         | `projectMembersService.addMember` → `assertPermission`; a body carrying `role` is refused 400 `role_retired` before the service (MOTIR-6464) | `member:manage`         | existing | R27 |
| `/api/projects/[key]/members/[userId]`                     | DELETE       | `projectMembersService.removeMember` → `assertPermission`                                                                                    | `member:manage`         | existing | R27 |
| `/api/projects/[key]/members/[userId]`                     | PATCH        | RETIRED — answers 410 `role_retired` without reading anything (MOTIR-6464)                                                                   | —                       | no-gate  | R72 |
| `/api/projects/[key]/roles`                                | POST         | RETIRED — answers 410 `project_roles_retired` without reading anything (MOTIR-6464)                                                          | —                       | no-gate  | R72 |
| `/api/projects/[key]/roles/[roleId]`                       | PATCH/DELETE | RETIRED — answers 410 `project_roles_retired` without reading anything (MOTIR-6464)                                                          | —                       | no-gate  | R72 |
...
| Operation                                           | Verbs        | Gate today                                                                                                                                                                                                                                                     | Permission                 | Decision         | Why |
| --------------------------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | ---------------- | --- |
| `/api/github/oauth/callback`                        | GET          | session only — binds the installation to a WORKSPACE; redirects to the surface the flow STARTED from (MOTIR-4676), defaulting to `/settings/organization/git`                                                                                                  | —                          | workspace-scoped | R3  |
| `/api/github/oauth/start`                           | GET          | session only — no project; redirects to the surface the flow STARTED from (MOTIR-4676), defaulting to `/settings/organization/git`                                                                                                                             | —                          | workspace-scoped | R3  |
...
| `/api/monitors/sentry/oauth/callback`               | GET          | session only — the httpOnly state cookie names the PROJECT the flow started on, where `integration:manage` was asserted; the service re-asserts it before any write, and a callback with no cookie stores nothing (MOTIR-5260)                                 | `integration:manage`       | new              | R66 |
| `/api/monitors/sentry/oauth/start`                  | GET          | `assertPermission(integration:manage)` on the `?project=` key — resolved and gated HERE, then carried in the cookie so the callback never reads a project id from a query string (MOTIR-5260)                                                                  | `integration:manage`       | new              | R66 |
| `/api/projects/[key]/monitors`                      | GET          | `assertPermission(integration:manage)` (`inProject`) — the room's read: the grant and this project's bindings                                                                                                                                                  | `integration:manage`       | new              | R66 |
| `/api/projects/[key]/monitors`                      | POST         | `assertPermission(integration:manage)` (`inProject`) — BIND one monitored project                                                                                                                                                                              | `integration:manage`       | new              | R66 |
| `/api/projects/[key]/monitors/[connectionId]`       | DELETE/PATCH | `assertPermission(integration:manage)` (`inProject`) — DELETE disconnects, removing the stored credential when it was the last binding; PATCH sets the binding's minimum level (MOTIR-5579)                                                                    | `integration:manage`       | new              | R66 |
| `/api/projects/[key]/monitors/available`            | GET          | `assertPermission(integration:manage)` — the picker's read; the one read on this surface that CALLS the provider                                                                                                                                               | `integration:manage`       | new              | R66 |
| `/api/projects/[key]/bug-destination`               | PATCH        | `assertPermission(project:administer)` in `bugDestinationService.setDestination`; another project's folder is `FolderNotFoundError` (MOTIR-4938)                                                                                                               | `project:administer`       | new              | R67 |
...
| Operation                                                        | Verbs       | Gate today                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Permission              | Decision    | Why |
| ---------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ----------- | --- |
| `/api/approval-gates/[id]/decide`                                | POST        | `approvalGatesService.decide` → `assertPermission` (the kind's floor) + assignee / reporter-if-unassigned / `approval:decide_any`                                                                                                                                                                                                                                                                                                                                                                                                  | `work_item:edit`        | new         | R64 |
| `/api/approval-gates/[id]/planning-seed`                         | GET         | `planningSeedService.getRefusalSeed` → `workItemsService.getWorkItem` (`assertCanBrowse` on the gate's work item, same project as the active one) + `isRefusalSeedGate`; `planChangeSessionsService.findSeededSession` (the caller's own session). Every refusal is one no-leak 404 (MOTIR-6208)                                                                                                                                                                                                                                   | `project:browse`        | new         | R2  |
| `approvalGatesService` records read (Approvals room)             | GET         | `listRecords` → browse floor; own records, or every record with `approval:view_any` via `getPermissions`                                                                                                                                                                                                                                                                                                                                                                                                                           | `approval:view_any`     | new         | R68 |
...
| `/api/workbench/stream`                                          | GET         | `requireCompliantWorkspaceContext` + the reader's own active project, BEFORE the stream opens; every access decision is the composed tab reads'                                                                                                                                                                                                                                                                                                                                                                                    | —                       | user-scoped | R70 |
...
| `/api/work-items/[id]/plans`                                     | GET         | `plansService.listPlanHistoryByWorkItemId` → `listPlanHistoryForWorkItem` → `assertPermission` (MOTIR-5546)                                                                                                                                                                                                                                                                                                                                                                                                                        | `ai:view_plan`          | existing    | R11 |
| `/api/work-items/[id]/pull-requests/[pullRequestId]/queue-again` | POST        | `pullRequestMergeService.{retryApproveAndMergeMember,requeueAutoMember}` → `assertPermission` (+ R64's relationship test on the gate path)                                                                                                                                                                                                                                                                                                                                                                                         | `work_item:edit`        | new         | R69 |
...
| `/api/work-items/approval-gate`                                  | GET         | `workItemsService.getWorkItemByIdentifier` → `assertCanBrowse`; then `approvalGatesService.getForWorkItem` (its `canDecide` asks `approval:decide_any`) and the kind's port: `designEvidenceService.getForGateSubject`, or for `pull_request_approval` (MOTIR-5439) the item page's own Development reads — `workItemsService.listLinkedPullRequests` / `getDeliveryView`, `howToTestService.getForWorkItem`, `designEvidenceService.getCurrentForWorkItem`, and `pullRequestMergeService.listApprovalMembers` on an approved gate | `project:browse`        | existing    | R2  |
...
| `/api/organization/repositories/[githubRepoId]`       | DELETE          | `assertOrgAdmin` in `organizationRepoService.disconnectFromOrganisation`, inside the transaction — the ORG-level disconnect. GitHub is refused 409 (Motir cannot remove one; the App's install screen owns it). Same answer as `settings/organization/security/actions.ts`: an ORG-ROLE gate, not a named permission                                                                                                         | —          | workspace-scoped | R3  |
...
| `/api/organizations/[orgId]/deletion`                 | GET/POST/DELETE | GET: `assertOrgMember` in `organizationDeletionService.getOrganizationDeletion` (non-member 404). POST/DELETE: `assertOrgCapability(…, 'deleteOrganization')` in `organizationDeletionService`, inside the transaction (MOTIR-6399): the Owner alone — non-member 404, Admin/Member 403; POST also needs the typed name and a fresh password step-up, and is rate-limited per user. An ORG-ROLE gate, not a named permission | —          | workspace-scoped | R3  |
...
| `/api/organizations/[orgId]/ownership-transfer`       | POST            | session only                                                                                                                                                                                                                                                                                                                                                                                                                 | —          | workspace-scoped | R3  |
| `/api/organizations/[orgId]/workspaces`               | GET             | `assertOrgCapability(…, 'manageWorkspaces')` in `workspacesService.listOrganizationWorkspaces`, inside the transaction (MOTIR-6309): non-member 404, Member 403. An ORG-ROLE gate from `lib/organizations/capabilities.ts`, not a named permission                                                                                                                                                                           | —          | workspace-scoped | R3  |
| `/api/organizations/[orgId]/workspaces/[workspaceId]` | DELETE          | `assertOrgCapability(…, 'manageWorkspaces')` in `workspacesService.removeWorkspaceAsOrgAdmin`, inside the delete's transaction (MOTIR-6309), with the workspace checked to belong to the URL's org (404 otherwise). An ORG-ROLE gate; no workspace membership needed                                                                                                                                                         | —          | workspace-scoped | R3  |
...
| `/api/workspaces/[workspaceId]/roles`                 | GET/POST        | GET: any workspace member (`readReachRole` non-null, else `NotAMemberError` → 404); POST: workspace ROLE Manager or the org Owner / an org Admin → `WorkspaceRoleForbiddenError` (MOTIR-6460)                                                                                                                                                                                                                                | —          | workspace-scoped | R3  |
| `/api/workspaces/[workspaceId]/roles/[roleId]`        | PATCH/DELETE    | workspace ROLE Manager or the org Owner / an org Admin → `WorkspaceRoleForbiddenError` (MOTIR-6460)                                                                                                                                                                                                                                                                                                                          | —          | workspace-scoped | R3  |
| `/api/workspaces/[workspaceId]/members/[userId]`      | PATCH           | workspace ROLE Manager or the org Owner / an org Admin → `WorkspaceRoleForbiddenError`; the last Manager is guarded under a row lock (`LastManagerError` → 409), and an org Owner / Admin target is refused (`OrgManagedWorkspaceRoleError` → 409) (MOTIR-6463)                                                                                                                                                              | —          | workspace-scoped | R3  |
...
| `app/(authed)/items/[key]/approvalGateActions.ts`        | decideApprovalGateAction                                                                                    | `assertPermission` (via the decide door)                                                                                                                                                                                    | `work_item:edit`     | existing         | R41 |
...
| `app/(authed)/settings/organization/actions.ts`          | reconcileActiveWorkspaceAction                                                                              | — none — (MOTIR-6312: session only; it writes nothing but the session's OWN active-workspace cookie, re-pointed to a workspace from the actor's own `listUserWorkspaces`. The removal it follows is the gated DELETE above) | —                    | workspace-scoped | R56 |
...
| `app/(authed)/settings/project/monitoring/actions.ts`    | recheckMonitorHealthAction                                                                                  | `monitorConnectionService.getView` + `monitorCredentialService.probeHealth` → `assertPermission` (MOTIR-5262)                                                                                                               | `integration:manage` | new              | R66 |
| `app/(authed)/settings/workspace/actions.ts`             | leaveWorkspaceAction, removeMemberAction, renameWorkspaceAction                                             | — none —                                                                                                                                                                                                                    | `work_item:edit`     | existing         | R41 |
| `app/(authed)/settings/organization/git/actions.ts`      | connectGitlabProjectAction, disconnectGitlabAction, disconnectGitlabProjectAction, listGitlabProjectsAction | — none —                                                                                                                                                                                                                    | `work_item:edit`     | existing         | R41 |
...
**R72.** THE RETIRED PROJECT-ROLE HANDLERS (Story MOTIR-6168 · MOTIR-6464; `role-model.md` §3). Roles live on the WORKSPACE, so a project membership carries no role and a project custom role grants nothing. Setting a member's project role (`PATCH …/members/[userId]`) and authoring, re-permissioning or deleting a project role (`…/roles`, `…/roles/[roleId]`) are gone; the handlers are kept as **410 Gone** stubs that name the workspace replacement (`/api/workspaces/{id}/members/{userId}`, `/api/workspaces/{id}/roles`) so a stale client reads a reason rather than a 404. **`no-gate` is literal and safe:** a stub reads no session, touches no table and changes nothing — there is no operation left to govern. R27 and R51 still describe these rows as they stood before, and the workspace equivalents are the R3 rows under `/api/workspaces/[workspaceId]`.
