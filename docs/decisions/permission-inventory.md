...
| `approval` (2)       | `approval:decide_any` (MOTIR-5292) · `approval:view_any` ᵖ (MOTIR-5305)                                                                  |
...
**It is ungrantable to an API token by derivation** (`lib/tokens/grant.ts`): a key is grantable because a token-reachable operation asserts it, and the room is a server-rendered page reading the service directly — no `/api/v1` route and no MCP tool asserts this key. **`planned` until MOTIR-5301**, which lands the read and flips it to `enforced` in the same change; until then the role screens list only enforced keys, so no switch renders for it.
...
| `approvalGatesService` records read (Approvals room)    | GET         | MOTIR-5301: `assertCanBrowse`; own records, or every record with `approval:view_any` via `getPermissions`                                                                                             | `approval:view_any`     | new      | R68 |
