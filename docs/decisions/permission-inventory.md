...
The two PROJECT-addressed reads (`…/projects/[key]/dispatch-runs/active`, `…/work-items/[key]/dispatch-runs`) resolve a project and assert **`project:browse`** through `projectAccessService`, exactly as every other project-addressed read does: a run history is project data, and whoever may see the project may see what ran in it.
