...
⚠️ **The READ is gated on the same key rather than on `project:browse`, and that is the one line in this row expected to move.** The sibling settings rooms split their verbs (`/api/projects/[key]/estimation-config` and `/api/projects/[key]/status-automation` both read on `project:browse`), and this one does not, because the room is offered to and reachable by exactly the actor who may write it. [MOTIR-5190](motir:cmtx85ynp00y6i0tx2lmi7cez) decided (2026-09-11) that the room is instead shown READ-ONLY to a member who cannot manage it, and [MOTIR-5193](motir:cmtx9wctp013uhvoil4s80ykf) builds the settings-registry VIEW key that makes that expressible. When it lands, this row's GET takes that view key and its PATCH keeps `workflow:manage`.
