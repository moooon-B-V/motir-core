...
**R55.** The REQUIRE-2FA policy, at the organization and workspace tiers (Story 8.13 · MOTIR-1215) — one switch per tier, demanding a second factor of everyone in it. Governed by the TIER ROLE (org-admin; `isWorkspaceManager`), asserted inside `twoFactorPolicyService`, and deliberately **not** by a permission-catalog key.
...
| `app/(authed)/settings/organization/security/actions.ts` | setOrganizationRequireTwoFactorAction                                             | `twoFactorPolicyService.setOrganizationPolicy` → org-admin         | —                    | workspace-scoped | R55 |
| `app/(authed)/settings/workspace/security/actions.ts`    | setWorkspaceRequireTwoFactorAction                                                | `twoFactorPolicyService.setWorkspacePolicy` → `isWorkspaceManager` | —                    | workspace-scoped | R55 |
