...
⚠️ **The GET is on `project:browse` and the PATCH is on `workflow:manage` — the room is SEEN by every project browser and CHANGED by an admin.** [MOTIR-5190](motir:cmtx85ynp00y6i0tx2lmi7cez) decided (2026-09-11) that the room is shown READ-ONLY to a member who cannot manage it; [MOTIR-5193](motir:cmtx9wctp013uhvoil4s80ykf) built the settings-registry VIEW key that makes that expressible; [MOTIR-5278](motir:cmtys0kdg00elhvoi3nesxn3w) made `approvals` its first declaration (`viewPermission: 'project:browse'`, `permission: 'workflow:manage'`) and moved this read with it. The split is the one the sibling settings rooms already make (`/api/projects/[key]/estimation-config` and `/api/projects/[key]/status-automation` both read on `project:browse`). **The two keys must stay the pair the registry declares**: a read re-gated on the write key admits a member through the rail onto a page whose read throws, and `tests/settings/projectSettingsNav.test.ts`'s view-key drift test reads `getSettings` so that edit fails there. A NON-browser is still answered `404` on either verb, before any key is consulted — the 404-vs-403 posture is unchanged.
...
| `/api/projects/[key]/approval-gates`                       | GET              | `assertPermission(project:browse)` in the service             | `project:browse`    | existing | R65 |
...
| `app/(authed)/workbench/approvalsActions.ts`             | loadApprovalSubjectAction                                                                                   | `getForGateSubject` (workspace-scoped, cross-card guarded)                                                    | `project:browse`     | existing         | R15 |
