...
| `system.code-graph-drift-sweep`             | `0,30 * * * *` | `latest`    | MOTIR-4644. `latest` because a missed tick is not work to REPLAY: the sweep selects from the CURRENT state of the estate — repositories whose `(indexedHeadSha, defaultBranchHeadSha)` pair has moved since it was last counted — so catching up on three skipped ticks would re-select the same rows three times and spend three provider calls to write one number. Not `skip`, because the count is what two surfaces render instead of an age, and a tick lost to a deploy should not leave the estate uncounted until the next one. Bounded per tick (`DRIFT_RECOMPUTE_BATCH`) against GitLab.com's 5 req/min ceiling, so a catch-up cannot fan out.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `system.code-graph-index-catch-up`          | `0,30 * * * *` | `latest`    | MOTIR-5290. `latest` because a missed tick is not work to REPLAY: each pass selects the repositories CURRENTLY paused by a hard stop of the internal index allowance and asks the CURRENT verdict, so replaying skipped ticks would ask the same question again and enqueue what the refresh debounce coalesces anyway. Not `skip`, because the tick is how a top-up, a renewed period or an upgrade reaches a paused repository at all — motir-core hears of none of them — and a tick lost to a deploy should not add thirty minutes to that. Bounded per tick (`INDEX_CATCH_UP_BATCH`), one ask per organisation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `system.dispatch-run-sweep`                 | `30 6 * * *`   | `latest`    | MOTIR-1792. Two convergent halves. The RETENTION half owes a published promise — a run started with `--report-log` says its log body is deleted after 30 days — so a missed night is a night of content we said was gone still sitting in a tenant's rows, the same class of commitment as the automation-retention and data-export sweeps. The REAP half is `system.job-run-reap`'s argument verbatim, one object out: another day of the run surface showing a dead run as `running` is the precise harm it exists to end. One pass covers every missed fire in both halves, because each candidate set is defined by ELAPSED TIME (`created_at < now − 30d`, `started_at < now − 12h`) rather than by the fire instant; replaying is free because a cleared body stops matching `body IS NOT NULL` and a closed run stops matching `status = 'running'`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `system.public-address-certificate-refresh` | `0,30 * * * *` | `latest`    | MOTIR-4219. A certificate's state changes on the PLATFORM and nothing in the request path can observe it, so a missed fire is a settings pane showing a stale `pending` for a domain that went live, or a confident `live` for one that expired. One pass sees everything every missed pass would have: the candidate set is every address whose status the job owns and whose `last_checked_at` is older than that status's staleness window, which is defined by ELAPSED TIME rather than by the fire instant. Replaying is free — the job REQUESTS nothing and derives every write from what the platform says now, so a second pass over a row it already reconciled writes the same answer. **⚠️ The cadence is BOTH clustered minutes, and it is the first schedule to take both.** MOTIR-4219 recommended five-minute polling; §21's clustering does not sell one, and a job needing finer granularity than thirty minutes is a decision to bring back there rather than a minute to pick. It costs NO new wake for that reason. What the thirty-minute floor costs is bounded: a customer actively watching their domain uses the pane's own _Check again_, which drives the lifecycle's verify path on demand — this job is the backstop for the customer who closed the tab, and for the renewal and the expiry nobody is watching. |
...
| `system.pull-request-reconcile`             | `0,30 * * * *` | `latest`    | MOTIR-5390. What it repairs is a card held at In Review by a merge whose `pull_request` delivery was lost — GitHub does not redeliver a failed App delivery, and nothing else re-reads a pull request from the host, so without a catch-up that card waits another half hour on top of an outage that may itself have been the cause. One pass sees everything every missed pass would have: the candidate set is every open, delivering row quiet for longer than a threshold, defined by ELAPSED TIME rather than by the fire instant. Replaying is free — a close replays through the delivery path, which converges on redelivery by construction, and a row it closed stops matching `state = 'open'`. Both clustered minutes, NO new wake.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `system.monitor-issue-reconcile`            | `0,30 * * * *` | `latest`    | MOTIR-4929 · MOTIR-5581. The monitor-issue reconciler's tick: it fans out one isolated poll per bound monitored project, and each poll reads every issue last seen AFTER that binding's stored watermark — defined by the stored watermark rather than by the fire instant, so one pass sees everything every missed pass would have, and a re-read is free because dedup is the provider's issue id under a row lock. Both clustered minutes, NO new wake.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `system.organization-deletion-reminders`    | `0 9 * * *`    | `latest`    | A missed morning is a reminder owed to an Owner about to lose an organization, and nothing else sends it — so it must not be skipped. One pass covers the gap because the candidate set is defined by elapsed time (`erasure_due_at` within seven days of now), and it sends only the SMALLEST threshold reached, so a request found late gets its one-day reminder rather than a stale seven-day one. Replaying is free: a sent reminder is recorded in `organization_deletion_notice` and never sent again.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `system.organization-erasure-sweep`         | `0 * * * *`    | `latest`    | A missed hour is an erasure owed against the date every member of the organization was emailed, and against the DPA §10 thirty-day promise, and nothing else erases it. One pass covers the gap because the candidate set is defined by elapsed time (`erasure_due_at <= now`, plus every request left `erasing`) rather than by the fire instant, and replaying is free: each step is idempotent and resumes from `erasure_step`, and an erased request stops matching.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `system.organization-retention-purge`       | `30 7 * * *`   | `latest`    | A missed day is one day of over-retention past the seven-year billing record (DECISION §7) and nothing else removes it. One pass covers the gap because the candidate set is defined by elapsed time (`erased_at` before `now − 7 years`) rather than by the fire instant, and replaying is free: a purged organization stops matching, and motir-ai answers `{ purged: false }` for one it already removed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `system.dlq-standing-depth-sweep`           | `0 7 * * *`    | `latest`    | MOTIR-5869. Files one bug per job function whose OLDEST unreplayed dead letter last failed more than seven days ago. The predicate is AGE over a standing table rather than anything about the fire instant, so one pass sees every function every missed pass would have; a missed day only delays a filing whose threshold is a week. Replaying is free because a filed function is disarmed under a row lock until its standing depth returns to zero. `07:00` is an hour of its own after the nightly cascade, on a clustered minute: NO new wake.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**Twenty `latest`, zero `skip`, zero `all`** (MOTIR-3314 moved the one `skip` — see its row; MOTIR-1115
added the weekly follower digest; MOTIR-3830 added the abandoned-supervision sweep; MOTIR-1792 added the
dispatch-run housekeeping). That
