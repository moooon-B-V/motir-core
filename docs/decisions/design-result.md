...
| The acceptance lane has **no `push:` trigger** (no post-merge republish) and **no `continue-on-error`** on its publish step                                       | `.github/workflows/acceptance-video.yml` (MOTIR-1937/1949; MOTIR-2499)                                                 |
...
| **Event**                    | **`pull_request` only** — `if: github.event_name == 'pull_request'`. `ci.yml` also runs on `push: main`; republishing an identical result after merge is the behaviour `acceptance-video.yml` deliberately avoids by having no `push:` trigger. Same decision, reached by a condition because the host job is shared.                                                                                           |
