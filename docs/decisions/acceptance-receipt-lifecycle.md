...
| The lane is `pull_request`-only and `paths:`-filtered to the spec files — deliberately, so a PR that owns no spec grows no greyed check | `.github/workflows/acceptance-video.yml` (MOTIR-1949 / MOTIR-1958)                                                  |
