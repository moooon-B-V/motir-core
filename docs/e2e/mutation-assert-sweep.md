...
The only (d) this suite has ever had is fixed on the SURFACE — and fixing it
left its three tests owing a wait on the WRITE:

| sites                                             | surface                                                       | the card                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issue-detail-flow.spec.ts:461` · `:595` · `:622` | `app/(authed)/items/[key]/_components/RelationshipsPanel.tsx` | **MOTIR-4496 fixed the surface**, merged 2026-09-04 in `motir-core#2626`: the panel is optimistic. **ARMED by MOTIR-5431.** This row used to say the panel inserts and removes its rows _from the action's own response_, and on that reading the in-page row assertion was the wait. **It is not:** `addLink` / `removeLink` apply the row BEFORE `createLinkAction` / `removeLinkAction` is sent, and `router.refresh()` reconciles afterwards. So each `goto` / `reload` that followed a row assertion raced the write — `:622` ejected `motir-core#2869` from the merge queue, the fourth sighting logged as a flake. Each test now arms `actionWrite(…)` on the link write and asserts its 200 before navigating. The optimistic assertions are unchanged, so they still detect MOTIR-4496's defect coming back |
