# Palette — Amethyst (`data-palette="amethyst"`)

> The warm palette Motir wore until MOTIR-6471, when it was called **Motir**
> (`data-palette="motir"`) and was the default. The monochrome palette formerly
> called Graphite took the name and the default ([Motir](motir.md)); this one kept
> every colour and took the name Amethyst. It is still the **Tier-3 base** in
> [`packages/design-system/theme.css`](../../packages/design-system/theme.css): it
> needs no root `[data-palette]` override block — the base `--el-*` layer _is_ the
> Amethyst palette. Registered in
...
**Inspiration:** Notion's warm marketing palette — Motir's house colours until
the monochrome rename.
...
the layer every component consumes. Amethyst's roles (light base; the Tier-1
...
Amethyst is the Tier-3 base, so its values are the `:root --el-*` block (which
reference the Tier-0 `--color-*` palette) — there is no root override block, only
a `[data-appearance-scope][data-palette='amethyst']` re-assertion for a nested
preview. Setting `data-palette="amethyst"` simply leaves the base `--el-*` tokens in force, exactly
...
Because Amethyst is the base palette, these steps live in the Tier-3 `:root` block
rather than a `[data-palette='amethyst']` override — `<html>` carries no
