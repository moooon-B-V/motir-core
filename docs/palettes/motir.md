...
> **Motir's own palette and the default** — a stark, editorial, **cool-neutral
> monochrome** re-skin. It shipped as **Graphite** (`data-palette="graphite"`,
> 7.3.51) as the cool foil to the warm palette then called Motir; MOTIR-6471
> gave it the product's name and made it the default, and the warm palette
> lives on as [Amethyst](amethyst.md). Only the name moved — every colour below
> is the one Graphite shipped. Registered in
> [`lib/theme/palettes.ts`](../../lib/theme/palettes.ts); its override lives in
> the **AXIS 1 (COLOUR)** section of
> [`packages/design-system/theme.css`](../../packages/design-system/theme.css) as
> the `[data-palette='motir']` block (light) + a
...
`data-style` axis — picking Motir never changes a radius. `data-theme`
...
Motir is the palette where the **chrome is greyscale**: surfaces, ink, and
...
var. So — exactly like the `[data-theme='dark']` block — Motir re-skins by
...
two read apart). Motir deliberately sets **`--color-info` = the accent blue**
...
and `--el-priority-high` rides `--color-warning` (`#c2410c`) — but Motir's
...
**priority ramp takes its own step of a hue Motir already owns**: **Radix
