...
// drives the `?peek=<identifier>` modal on every surface it appears (/issues,
...
// server page), then it client-FETCHES the item's fields from /api/issues/peek
...
export function IssueQuickViewController() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/issues/peek?key=${encodeURIComponent(peek)}`, {
