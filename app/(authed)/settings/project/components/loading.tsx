// Loading skeleton for the Components settings page (the 5.4.7 mockup, panel
// 7: header + row skeletons extending the settings skeleton grammar). Pure
// presentational pulse blocks — colour through --el-* fills only.

export default function ComponentsSettingsLoading() {
  return (
    <div className="mx-auto flex max-w-[42rem] animate-pulse flex-col gap-6" aria-hidden>
      <header className="flex flex-col gap-2">
        <div className="bg-(--el-muted) h-8 w-40 rounded-(--radius-control)" />
      ...
      <div className="border-(--el-border) rounded-(--radius-card) border p-(--spacing-card-padding)">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="bg-(--el-muted) h-7 w-28 rounded-(--radius-btn)" />
        </div>
