...
import { DATA_PRIVACY_MAILBOX } from '@/lib/users/dataSubjectRequests';
...
// `design/settings/design-notes.md` → `Data & privacy`. This card builds panels
// 1 (at rest), 2 (the export's states), 4 (BLOCKED) and 6 (dark parity). The
// confirmation ledger, the scheduled state and the app-wide cancel banner are
// MOTIR-3704.
//
...
async function DataPane({ userId, email }: { userId: string; email: string }) {
  const [preview, exportRequest] = await Promise.all([
  ...
  return (
    <div className="flex flex-col gap-6">
      <DataExportCard request={exportRequest} email={email} />
      <DeleteAccountCard
        blocked={preview.blocked}
        blockingOrganization={preview.blockingOrganization}
      />
      ...
      {preview.blocked && soleMemberCount > 0 ? (
        <SettingsCallout>
