...
// quick-view eye PLUS the shared ⋯ actions menu (Edit details · Copy link ·
// Archive · Delete…). Caps come from `useProjectAccess()` (resolved once in the
// authed layout), so the menu gates Archive on `canEdit` and Delete on
// `canManage` without per-row threading. Page state after a delete/archive: bump
// the `issuesChangedAt` tick — the Tree/List is a client island (IssueTreeTable)
// that refetches on it, the page-state-after-mutation contract for an island.
export function WorkItemRowActions({ row }: { row: IssueRowData }) {
  ...
  return (
    <span className="flex items-center justify-end gap-0.5">
      <QuickViewTrigger identifier={row.identifier} title={row.title} />
      <WorkItemActionsMenu
        itemId={row.id}
