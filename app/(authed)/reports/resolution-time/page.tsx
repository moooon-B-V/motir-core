...
import {
  ...
  type ResolutionTimeResult,
} from '../_components/ResolutionTimeReport';

// The Resolution-time report page (Story 8.8 · Subtask 8.8.13) — per
// design/reports/more-reports.mock.html panel 3. Server Component: it resolves
...
export default async function ResolutionTimePage({
  searchParams,
}: {
  ...
  const [result, filtersPage] = await Promise.all([
    reportsService.getResolutionTime(scope, { period, daysBack }, accessCtx),
    savedFiltersService.list(ctx.project.identifier, { view: 'all' }, accessCtx),
  ]);

  const clientResult: ResolutionTimeResult =
    result.state === 'ok'
  ...
  const subLine = [
    ...
    t('resolutionTime.subWindow', { days: daysBack }),
    windowAverage !== null ? t('resolutionTime.subAverage', { days: windowAverage }) : '',
  ]
    .filter(Boolean)
    .join(' · ');
