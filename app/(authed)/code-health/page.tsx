...
export default async function CodeHealthPage() {
  ...
  // A project with no connected repo → no calls (empty surface, the fresh state).
  ...
  if (repos.length > 0) {
    try {
      // Call per repo — show the FIRST repo's convention for now; multi-repo
      // listing (one card per repo) is the next increment (MOTIR-1663 follow-up).
