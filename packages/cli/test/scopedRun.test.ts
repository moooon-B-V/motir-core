...
// ⚠️ `openChildrenHoldReason` / `renderOpenChildrenHold` HAD SUITES HERE, and
// MOTIR-4967 retired both with the `held` outcome they rendered. A container with
// an unlanded child no longer withholds its pull request — it leaves the DRAFT
// one it already has unready — so there is no hold sentence to mirror and no
// three-way disposition for an operator to pick from. `childrenBelowClaimBar`
// above is unchanged and still computes the set; what the summary prints from it
// is covered in `test/auto.test.ts`.
