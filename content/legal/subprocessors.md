...
| **Inngest**                                       | **NO CHAPTER V MECHANISM, and none will be sought — the dependency is being REMOVED instead.** Inngest's Terms of Service and Privacy Policy carry **no DPA, no SCCs and no international-transfer language at all**; its Trust Center is access-gated. Its **SOC 2 Type II** is a security-control audit and is **not** a Chapter V transfer mechanism — named here to be refused explicitly, because the two are routinely confused. **The remedy is retirement, not a vendor agreement:** MOTIR-3413 replaces it with an in-product Postgres queue, and MOTIR-3418 deletes the SDK. **Inngest is expected to leave this list before the service is generally available**, because the replacement is well advanced and launch is not. | Inngest's Terms and Privacy Policy, read 2026-08-27; remedy set by MOTIR-3628 |
...
- **Inngest — OPEN, and closing by REMOVAL rather than by agreement.** A core
  ...
  **This row is expected to be deleted rather than resolved**, and MOTIR-3418 owns
  that deletion — its own acceptance now names this file. Nothing gates the pages
  on it: the service is not generally available, so building the routes publishes
  nothing to anyone, and the replacement is far enough advanced that the row
  should be gone before that changes.
