...
| -------------------------------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sentry** (Functional Software, Inc.)             | Error monitoring                  | **Receives nothing today — but it IS configured.** `SENTRY_DSN` and `SENTRY_ENVIRONMENT=production` are set on the running `motir-core` application and address a US Sentry ingest host. No Sentry SDK exists on `origin/main`, so no code sends to it; the wiring is written and unmerged. **It begins receiving on the merge, and this page is amended in the same change**                                                                                                                                          |
...
Stripe that "no SDK dependency exists". Sentry's configuration exists on the
...
| **Inngest**                                       | **STILL NOT CONFIRMED, and now with a reading rather than a search.** Inngest's Terms of Service and Privacy Policy carry **no DPA, no SCCs and no international-transfer language at all**; its Trust Center is access-gated and publishes nothing readable. Its **SOC 2 Type II** attestation is a security control audit and is **not** a Chapter V transfer mechanism — the two are routinely confused, so it is named here to be refused explicitly. This is a core subprocessor that handles job payloads: it cannot stay unresolved, and the next step is to REQUEST a DPA from the vendor, not to search again | Inngest's Terms and Privacy Policy, read 2026-08-27; Trust Center publishes nothing public |
...
- **Inngest — OPEN.** A core subprocessor, receiving job payloads that reference
  work items and may carry their content, with **no published Chapter V transfer
  mechanism of any kind**. Its SOC 2 Type II attestation does not answer this.
  The next step is to request a DPA from the vendor; searching again will not
  change the answer.
...
| `motir-core` — dependency manifest                                          | **Tigris** (S3 SDK) · **Inngest** · no Sentry SDK · no Stripe SDK                                                                                |
